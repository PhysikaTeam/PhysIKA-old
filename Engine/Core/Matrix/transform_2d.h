...
public:
    ...
    //Get Variable
    inline Scalar rotateAngle() const { return this->rotate_angle_; }
    inline Vector<Scalar, 2> translation() const { return this->translation_; }
    inline Vector<Scalar, 2> scale() const { return this->scale_; }
    
    //Set
    inline void setTranslation(const Vector<Scalar, 2> &translation) { this->translation_ = translation; }
    inline void setRotationAngle(const Scalar &rotate_angle) { this->rotate_angle_ = rotate_angle; }
    inline void setScale(const Vector<Scalar, 2> &scale) { this->scale_ = scale; }
    inline void setRotation(const SquareMatrix<Scalar, 2> &rotation) { this->rotate_angle_ = acos(rotation(0,0));} 
    inline void setRotation(const SquareMatrix<Scalar, 3> &rotation) { this->rotate_angle_ = acos(rotation(0,0));}
    inline void setIdentity() { this->rotate_angle_ = 0; this->translation_ = Vector<Scalar, 2>(0); this->scale_ = Vector<Scalar, 2>(1);}
