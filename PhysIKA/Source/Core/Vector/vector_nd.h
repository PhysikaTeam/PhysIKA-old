...
namespace PhysIKA
{
	...
	public:
		...
		Vectornd(const Vectornd<T, deviceType>& v):m_n(0), m_data(NULL)
		{
		...
		void resize(int n);
		COMM_FUNC virtual int size() const { return m_n; }
		//virtual int size()const = 0;


		COMM_FUNC inline T& operator()(unsigned int i) { return m_data[i]; }
		COMM_FUNC inline const T& operator()(unsigned int i)const { return m_data[i]; }
		COMM_FUNC inline T& operator[] (unsigned int i) { return m_data[i]; }
		COMM_FUNC inline const T& operator[] (unsigned int i) const { return m_data[i]; }
