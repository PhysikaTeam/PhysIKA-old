#include "Physika_Surface_Fuild/CUDAII/swe.cuh"
#include <cuda.h>
#include <cuda_runtime_api.h>
#include <cuda_runtime.h>
#include <device_launch_parameters.h>
namespace Physika{
void Simulator2::init(int argc, char** argv) {
...
void Simulator2::run_cuda ( int frame )
{
	cudaError_t error = cudaGetLastError();
	if (error != cudaSuccess) {
...
void Simulator2::output_obj_cuda(int frame) {
...
void Simulator2::clear() {
	release_index();
	release_properties();
}
void Simulator2::set_initial_constants(){
...
void Simulator2::generate_origin(){
  m_origin.clear();
...
void Simulator2::generate_mesh(){
    m_mesh.clear();
    m_mesh = m_origin; jacobi_laplace_smooth_and_expand(m_mesh, 50);
}
void Simulator2::add_properties() {
...
void Simulator2::calculate_tensor() {
...
void Simulator2::set_initial_conditions(){
    for (auto v_it = m_mesh.vertices_begin(); v_it != m_mesh.vertices_end(); ++v_it) {
...
void Simulator2::output_obj() {
	output_obj_cuda(m_stepnum);
}
void Simulator2::advect_filed_values() {
	for (auto v_it = m_mesh.vertices_begin(); v_it != m_mesh.vertices_end(); ++v_it) {
...
void Simulator2::release_properties() {
