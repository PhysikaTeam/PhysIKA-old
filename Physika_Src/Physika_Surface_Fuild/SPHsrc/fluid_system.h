...
*/
	...
	#define PLAMBDA_EPS			48
	#define PVORT_EPS			49
	#define PSCENE_NUM			50
	#define PMOLE_RATIO			51
	#define PTIME_EXTFORCE		52
	#define PTIME_ITERATION		53
	#define PTIME_UPDATEV		54
	...
	public:
		...
		// Output
		void output_particles(int frame);
		...
		void SetupSpacing ();
		void SetupAddParticle ();
		void SetupAddVolume ( Vector3DF min, Vector3DF max, float spacing, float offs, int total, int type, mfloat mratio, int phase );
		void SetupGridAllocate ( Vector3DF min, Vector3DF max, float sim_scale, float cell_size, float border );
		int ParseXMLFluid ( std::string name, int id, bool bStart );
		void ParseXMLScene ( std::string name, int id );
		void ReadBi2 ( char* filename, int type, float mratio );
		...
		void setForceType ( int ftype )		{ mForceType = ftype; }
		bool getShow () { return mShow; }
		void setShow ( bool flag ) { mShow = flag; }

		//SWE
		void UpdateFromSWE();
		void AddInfluenceToSWE();
		void AddEmitFull(float spacing, int cat, float mass);
		void EmitParticlesFull(float spacing, int cat, float mass);
	...
	private:
		...
		Vector3DF					m_SceneVec[MAX_SCENE][MAX_PARAM];
		mfloat						malpha[MAX_SCENE][MAX_PARAM];
