...
//
	...
	public:
		...
		Vector4DI &Set ( const int xa, const int ya, const int za )	{ x = xa; y = ya; z = za; w = 1; return *this; }
		Vector4DI &Set ( const int xa, const int ya, const int za, const int wa )	{ x = xa; y = ya; z = za; w = wa; return *this; }

		// Constructors/Destructors
		Vector4DI () { x = 0; y = 0; z = 0; w = 0; }
		Vector4DI ( const VTYPE xa, const VTYPE ya, const VTYPE za, const VTYPE wa );

		// Member Functions
		Vector4DI &operator= (const int op);
		Vector4DI &operator= (const double op);

		Vector4DI &operator+= (const int op);
		Vector4DI &operator+= (const float op);
		Vector4DI &operator+= (const double op);
		...
		// Slow operations - require temporary variables
		Vector4DI operator+ (const int op)			{ return Vector4DI ( x + op, y + op, z + op, w*op ); }
		Vector4DI operator+ (const float op)		{ return Vector4DI ( x + op, y + op, z + op, w*op ); }
		Vector4DI operator- (const int op)			{ return Vector4DI ( x - op, y - op, z - op, w*op ); }
		Vector4DI operator- (const float op)		{ return Vector4DI ( x - op, y - op, z - op, w*op ); }
		Vector4DI operator* (const int op)			{ return Vector4DI ( x*op, y*op, z*op, w*op ); }
		Vector4DI operator* (const float op)		{ return Vector4DI ( x*op, y*op, z*op, w*op ); }
		...
		Vector4DI &Set ( CLRVAL clr )	{
			x = (int)RED ( clr );		// (float( c      & 0xFF)/255.0)	
			y = (int)GRN ( clr );		// (float((c>>8)  & 0xFF)/255.0)
			z = (int)BLUE ( clr );		// (float((c>>16) & 0xFF)/255.0)
			w = (int)ALPH ( clr );		// (float((c>>24) & 0xFF)/255.0)
			return *this;
		}
		Vector4DI& fromClr ( CLRVAL clr ) { return Set ( clr ); }
		CLRVAL toClr () { return (CLRVAL)COLORA ( x, y, z, w ); }

		Vector4DI& Clamp ( float xc, float yc, float zc, float wc )
		{
			x = (x > xc) ? xc : x;
			y = (y > yc) ? yc : y;
			z = (z > zc) ? zc : z;
		...
		Vector4DI &Cross ( const Vector4DI &v );

		double Dot ( const Vector4DI &v );

		double Dist ( const Vector4DI &v );

		double DistSq ( const Vector4DI &v );

		Vector4DI &Normalize ( void );
		double Length ( void );

		Vector4DI &Random ()		{ x = int ( rand () ) / RAND_MAX; y = int ( rand () ) / RAND_MAX; z = int ( rand () ) / RAND_MAX; w = 1;  return *this; }
