...
nvImg::nvImg (int width_, int height_)
{
	create ( width_, height_, 3 );  
}


nvImg::nvImg (int width_, int height_, int channels_)
{
	create (width_, height_, channels_ );
}

void nvImg::create ( int width_, int height_, int channels_ )
{
	  width    = width_;
	  height   = height_;
	  channels = channels_;
...
void nvImg::draw ( float x, float y )
{	
...
nvImg::nvImg (int width_, int height_, int channels_, int bits_)
{
  width    = width_;
  height   = height_;
  channels = channels_;
...
nvImg::nvImg (const char* filename)
{
  width    = 0;
  height   = 0;
  channels = 0;
...
nvImg& nvImg::operator= (const nvImg& image)
{
...
void nvImg::setPixels ( unsigned char *newPixels )
{
  if( bad() || ! newPixels )
    return;
...
int nvImg::index (int x, int y, int c)
{
  return (((height - y - 1) * width + x) * channels + c);
}


double nvImg::getPixel (int x, int y, int channel)
{
  assert(good());
  assert((x >= 0)       &&
         (x <  width)   &&
...
double nvImg::getPixel_ (int x, int y, int channel)
{
  if (!good()        ||
      (x <  0)       ||
      (x >= width)   ||
...
Pixel nvImg::getPixel (int x, int y)
{
  assert(good());
  assert((x >= 0)       &&
         (x <  width)   &&
...
Pixel nvImg::getPixel_ (int x, int y)
{
  if (!good()        ||
      (x <  0)       ||
      (x >= width)   ||
...
Pixel& nvImg::getPixel (int x, int y, Pixel& pixel)
{
  assert(good());
  assert((x >= 0)       &&
         (x <  width)   &&
...
Pixel& nvImg::getPixel_ (int x, int y, Pixel& pixel)
{
  if (!good()        ||
      (x <  0)       ||
      (x >= width)   ||
...
void nvImg::setPixel (int x, int y, int channel, double value)
{
  assert(good());
...
void nvImg::setPixel_ (int x, int y, int channel, double value)
{
  if (!good()        ||
      (x <  0)       ||
      (x >= width)   ||
...
void nvImg::setPixel4 ( int x, int y, Pixel pixel )
{
	setPixel ( x, y, pixel );
...
void nvImg::setPixel (int x, int y, Pixel pixel)
{
  assert(good());
...
void nvImg::setAlpha (int x, int y, double value)
{
	assert(good());
...
void nvImg::setPixel_ (int x, int y, Pixel pixel)
{
  if (!good()        ||
      (x <  0)       ||
      (x >= width)   ||
...
void nvImg::glReadPixelsWrapper ()
{
  assert(good());
...
void nvImg::glDrawPixelsWrapper ()
{
  assert(good());
...
void nvImg::glTexImage2DWrapper ()
{
  assert(good());
...
void nvImg::glTexImageCubeWrapper ( int i )
{
  assert(good());
...
void nvImg::glTexSubImage2DWrapper ( int x, int y)
{
  assert(good());
...
int nvImg::read (const char* filename )
{
	return read ( filename, 0x0 );
}

int nvImg::read (const char* filename, const char* alphaname )
{
	FILE *file, *file_a;
...
int nvImg::write (const char* filename)
{
...
//
	...
	int nvImg::readJPG (const char* filename)
	{
	...
	int nvImg::writeJPG (const char* filename)
	{
	  FILE* file = fopen(filename, "wb");

	  if (!file)
	  {
		perror("nvImg::writeJPG");
		return -1;
	  }
	...
	int nvImg::readJPG (FILE* file)
	{
	  ...
	  if (setjmp(jerr.setjmp_buffer)) 
	  {
		...
		fprintf(stderr, "nvImg::readJPG: jpeg decompression error");
		return -1;
	  }
	  ...
	  {
		fprintf(stderr, "nvImg::readJPG: unrecognized colorspace");
		return -1;
	  }
	...
	int nvImg::writeJPG (FILE* file)
	{
	  if (!good())
	  {
		fprintf(stderr, "nvImg::writeJPG: bad image\n");
		return -1;
	  }
...
int nvImg::readBMP (const char* filename )
{
  FILE* file = fopen(filename, "rb");
  if (!file)
  {
    perror("nvImg::readBMP");
    return -1;
  }
...
int nvImg::writeBMP (const char* filename)
{
  FILE* file = fopen(filename, "wb");

  if (!file)
  {
    perror("nvImg::writeBMP");
    return -1;
  }
...
// read BMP palette
int nvImg::readPaletteBMP ( FILE* fp, RGBQUAD*& palette, int bit_count )
{
	bool bColor;
	int palSize = 0;
	switch ( bit_count ) {
...
int nvImg::readBMP (FILE* fp, FILE* fp_a, bool bBaseImg  )
{
	RGBQUAD*	palette;
	int			palSize;
	int         index;
    ...
    if (bmfh.bfType != BMP_BF_TYPE) {
      fprintf( stderr, "nvImg::readBMP: unrecognized type\n" );
      return -1;
    }
    ...
    if ((bmih.biWidth <= 0)  || (bmih.biHeight <= 0) || (bmih.biPlanes != 1)) {
		fprintf(stderr, "nvImg::readBMP: malformed file\n");
		return -1;
    }
...
int nvImg::writeBMP (FILE* fp)
{
    if (!good())
    {
      fprintf(stderr, "nvImg::writeBMP: bad image\n");
      return -1;
    }
...
int nvImg::readPNM (const char* filename)
{
  FILE* file = fopen(filename, "rb");

  if (!file)
  {
    perror("nvImg::readPNM");
    return -1;
  }
...
int nvImg::writePNM (const char* filename)
{
  FILE* file = fopen(filename, "wb");

  if (!file)
  {
    perror("nvImg::writePNM");
    return -1;
  }
...
int nvImg::readPNM (FILE* fp)
{
  ...
  if (nextLine[0] == 'P')
  {
    ...
    default:
      fprintf( stderr, "nvImg::readPNM: malformed file\n" );
      return -1;
    }
  }
  else
...
int nvImg::writePNM (FILE* fp)
{
  if (!good())
  {
    fprintf(stderr, "nvImg::writePNM: bad image\n");
    return -1;
  }
