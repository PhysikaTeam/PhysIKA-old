...
void XmlSettings::assignValueM ( mfloat* buf, std::string key )
{
	TiXmlElement* elem = (*mCurrBase).ToNode ()->FirstChildElement ( key );
	if (elem != 0x0 && elem->GetText () != 0x0) {
		*buf = ParseMfloat ( elem->GetText () );		// Assignment only occurs if xml element exists
	}
...
mfloat XmlSettings::ParseMfloat ( std::string mf )
{
	mfloat buf;
	for (int i = 0; i < TYPE_NUM; i++) buf.tid[i] = 0;

	if (mf.length () == 0) return buf;
	str_vec_t vals = StringHelper::SplitString ( mf, ',' );

	for (int i = 0; i < TYPE_NUM; i++)
		buf.tid[i] = atof ( vals[i].c_str() );
