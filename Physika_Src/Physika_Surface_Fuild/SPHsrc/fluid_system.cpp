...
{ 
#ifndef HYBRID
	CopyToCUDA ( (float*)mPos, (float*)mVel, (float*)mVelEval, (float*)mForce, mPressure, mDensity, mClusterCell, mGridNext, (char*)mClr, mType, mMass, mRestDens, mAlpha );
#else
	CopyToCUDA((float*)mPos, (float*)mVel, (float*)mVelEval, (float*)mForce, mPressure, mDensity, mClusterCell, mGridNext, (char*)mClr, mType, mMass, mRestDens, mAlpha, mSWEvertex);
#endif
...
{
	setForceType ( 0 );
	CopyFromCUDA ( (float*) mPos, (float*) mVel, (float*) mVelEval, (float*) mForce, mPressure, mDensity, mClusterCell, mGridNext, (char*) mClr, mGridCell, mType, mMass, mAlpha );

	mNumPoints = GetSPHCurrentPnum();
	//char filename[255];
	//if (m_Frame <= 1436 && m_Frame%4==0){
	//	sprintf ( filename, "../newData/3/frame_%04d.txt", m_Frame );
	//	SaveData ( filename ); // when save data to file, do remember copy the mtype from CUDA.
	//	if (m_Frame>=1436) exit ( 0 );
	//}
}

void FluidSystem::SaveData ( char* filename )
{
	FILE* fp = fopen ( filename, "w" );
	fprintf ( fp, "%d\n", mNumPoints - mNumBound );
	for (int i= 0; i < mNumPoints; i++){
		if (mType[i]==0 /*&& mPos[i].y>9.0*/)
			fprintf ( fp, "%f %f %f %f %f %d\n", mPos[i].x, mPos[i].y, mPos[i].z, mAlpha[i].tid[0], mAlpha[i].tid[1], mType[i] );
	}
	fclose ( fp );
}
...
#ifdef CONSOLE
		printf ( "fluid", "ERROR: Problem loading scene.xml. Check formatting.\n" );
#else
...
void FluidSystem::Setup ( bool bStart )
{
	...
	maxAllowedPoints = NumPoints()*ADD_EXTRA_BUF_RATE;
		...
		FluidSetupCUDA ( NumPoints(), mNumSolid, m_GridSrch, *(int3*)& m_GridRes, *(float3*)& m_GridSize, *(float3*)& m_GridDelta, *(float3*)& m_GridMin, *(float3*)& m_GridMax, m_GridTotal, (int) m_Vec[PEMIT_RATE].x );

		Sleep ( 500 );

		Vector3DF grav = m_Vec[PPLANE_GRAV_DIR];
		FluidParamCUDA ( mpmass, m_Param[PSIMSCALE], m_Param[PSMOOTHRADIUS], m_Param[PRADIUS], m_Param[PMASS], m_Param[PRESTDENSITY], *(float3*)& m_Vec[PBOUNDMIN], *(float3*)& m_Vec[PBOUNDMAX], m_Param[PEXTSTIFF], m_Param[PINTSTIFF], m_Param[PVISC], m_Param[PEXTDAMP], m_Param[PFORCE_MIN], m_Param[PFORCE_MAX], m_Param[PFORCE_FREQ], m_Param[PGROUND_SLOPE], grav.x, grav.y, grav.z, m_Param[PACCEL_LIMIT], m_Param[PVEL_LIMIT], m_Param[PLAMBDA_EPS], m_Param[PVORT_EPS] );
...
void FluidSystem::AllocateParticles ( int cnt )
{
	...
	int* srcType = mType;
	mType = (int*) malloc ( cnt*sizeof ( int ) );
	if (srcType != 0x0) { memcpy ( mType, srcType, nump*sizeof ( int ) ); free ( srcType ); }

	float* srcMass = mMass;
	mMass = (float*) malloc ( cnt*sizeof ( float ) );
	if (srcMass != 0x0) { memcpy ( mMass, srcMass, nump*sizeof ( float ) ); free ( srcMass ); }

	mfloat* srcAlpha = mAlpha;
	mAlpha = (mfloat*) malloc ( cnt*sizeof ( mfloat ) );
	if (srcAlpha != 0x0) { memcpy ( mAlpha, srcAlpha, nump*sizeof ( mfloat ) ); free ( srcAlpha ); }

	float* srcRestDens = mRestDens;
	mRestDens = (float*) malloc ( cnt*sizeof ( float ) );
	if (srcRestDens != 0x0) { memcpy ( mRestDens, srcRestDens, nump*sizeof ( float ) ); free ( srcRestDens ); }
...
#ifdef HYBRID
	int* srcSWEVertex = mSWEvertex;
	mSWEvertex = (int*)malloc(cnt*sizeof(int));
	if (srcSWEVertex != 0x0){ memcpy(mSWEvertex, srcSWEVertex, nump*sizeof(int)); free(srcSWEVertex); }
#endif
...
int FluidSystem::AddParticle ( int type)
{
...
void FluidSystem::SetupAddVolume ( Vector3DF min, Vector3DF max, float spacing, float offs, int total, int type, mfloat mratio, int phase )
{
	Vector3DF pos;
	int n, p;
	...
	for (float y = min.y; y <= max.y; y += spacing ) {	
		for (int xz=0; xz < cnt; xz++ ) {
			...
			if ( p != -1 ) {
				(mPos + p)->Set (x, y, z);
				for (int i = 0; i < TYPE_NUM; i++) {
					*(mMass + p) += mratio.tid[i] * mpmass.tid[i];
					*(mRestDens + p) += mratio.tid[i] * mprest_dens.tid[i];
				}
				for (int i = 0; i < TYPE_NUM; i++)
					(mAlpha + p)->tid[i] = mratio.tid[i] * mpmass.tid[i] / *(mMass + p);
				switch (phase){
				case 0: 
...
void FluidSystem::SetupAddParticle ()
{
#ifndef HYBRID
	for (int i = 0; i < (int)m_Param[PSCENE_NUM]; i++){
		SetupAddVolume ( m_SceneVec[i][PINITMIN], m_SceneVec[i][PINITMAX], m_Param[PSPACING], 0.1, m_Param[PNUM], 0, malpha[i][PMOLE_RATIO], i);
	}
	//ReadData ( "still.txt" );
	
	//ReadBi2 ( "objs/bi2/bound.bi2", -1, 1.0 );
	ReadBi2 ( "objs/bi2/Beaker.bi2", -1, 1.0 );
	//ReadBi2 ( "objs/bi2/Beaker1.bi2", -1, 1.0 );
	ReadBi2 ( "objs/bi2/Hockey_Stick.bi2", -2, 1.0 );
	//ReadBi2 ( "objs/bi2/bottom.bi2", -1, 2.0 );
	//ReadBi2 ( "objs/bi2/top.bi2", -1, 2.0 );
	//ReadBi2 ( "objs/bi2/Seperating_Funnel.bi2", -1, 2.0 );
#else

	SetupAddVolume(m_SceneVec[0][PINITMIN], m_SceneVec[0][PINITMAX], m_Param[PSPACING], 0.1, m_Param[PNUM], 0, malpha[0][PMOLE_RATIO], 0);
	for (int i = 0; i < sim.m_mesh.n_vertices(); i++){
		int p = AddParticle(-3);
		if (p != -1){
			(mPos + p)->Set(sim.c_point[i].x,sim.c_point[i].y,sim.c_point[i].z);
			*(mClr + p) = COLORA(1, 1, 1, 1);
			*(mMass + p) = m_Param[PMASS];
			*(mRestDens + p) = m_Param[PRESTDENSITY];
#ifdef HYBRID
			*(mSWEvertex + p) = i;
#endif
		}
	}
	//ReadBi2("objs/bi2/arma.bi2", -3, 1.0);
#endif
}

void FluidSystem::ReadData ( char* filename )
{
	...
	for (int i = 0; i < pnum; i++){
		...
		if (p != -1){
			*(mPos + p) = pos;
			if (pos.y < 98) {
				*(mMass + p) = 2 * m_Param[PMASS];
				*(mRestDens + p) = 2 * m_Param[PRESTDENSITY];
				(mAlpha + p)->tid[0] = 0.25; (mAlpha + p)->tid[1] = 0.75; (mAlpha + p)->tid[2] = 0.0;
				*(mClr + p) = COLORA ( 1, 1, 0, 1 );
			}
			else {
				*(mMass + p) = 2 * m_Param[PMASS];
				*(mRestDens + p) = 2 * m_Param[PRESTDENSITY];
				(mAlpha + p)->tid[0] = 0.0; (mAlpha + p)->tid[1] = 0.0; (mAlpha + p)->tid[2] = 1.0;
...
void FluidSystem::ReadBi2 ( char* filename, int type, float mratio )
{
	BI2Reader bi2reader ( filename );
	bi2reader.GetInfo ( false );

	for (int i = 0; i < bi2reader.info.nfixed; i++){
		int p = AddParticle ( type );
		if (p != -1){
			(mPos + p)->Set ( bi2reader.info.Pos[i].x, bi2reader.info.Pos[i].y, bi2reader.info.Pos[i].z );
			*(mClr + p) = COLORA ( 1, 1, 1, type<0? 1:1 );
			*(mMass + p) = mratio * m_Param[PMASS];
			*(mRestDens + p) = mratio * m_Param[PRESTDENSITY];
		}
	}
}
...
void FluidSystem::RunSimulateCUDAPBD ()
{
	Time start;
	...
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-2.0: %s\n", cudaGetErrorString(error));
	}
	
	start.SetSystemTime ();
	ComputeExternalForceCUDA ( m_DT, m_Frame, mForceType );
	record ( PTIME_EXTFORCE, "External Force CUDA", start );
	start.SetSystemTime ();
#else
	//EmitParticlesFull(0, 0, 0);
#endif

#ifdef NEW_SORT
	error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-2: %s\n", cudaGetErrorString(error));
	}
	InitialSortCUDA ( 0x0, 0x0, 0x0 );
	error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-3: %s\n", cudaGetErrorString(error));
	}
	SortGridCUDA ( 0x0 );
	error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-4: %s\n", cudaGetErrorString(error));
	}
	CountingSortFullCUDA_ ( 0x0 );
	 error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-1: %s\n", cudaGetErrorString(error));
	}
#else
	InsertParticlesCUDA ( 0x0, 0x0, 0x0 );
	PrefixSumCellsCUDA ( 0x0 );
...
#ifndef HYBRIDSPHSIM
	start.SetSystemTime ();
	//StablizationIterationCUDA ( 2, m_Time, m_DT);
	SolverIterationCUDA ( 20, m_Time, m_DT );
	record ( PTIME_ITERATION, "Iteration CUDA", start );
	error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-1.1: %s\n", cudaGetErrorString(error));
	}
	start.SetSystemTime ();
	UpdateVelocityCUDA ( m_DT );
	record ( PTIME_UPDATEV, "UpdateV CUDA", start );
	error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-1.2: %s\n", cudaGetErrorString(error));
	}
	UpdateParticlePositionCUDA ( m_Frame );
	error = cudaGetLastError();
	if (error != cudaSuccess) {
		fprintf(stderr, "CUDA ERROR: FSInitialAdd-1.3: %s\n", cudaGetErrorString(error));
	}
#endif
#ifdef HYBRIDSPHSIM
	//PERF_PUSH("PressureCUDA");
...
void FluidSystem::Run (int width, int height)
{
	...
	case RUN_CUDA_PBD:		RunSimulateCUDAPBD ();	break;
	};
...
void FluidSystem::Draw ( Camera3D& cam, float rad )
{
	...
	case 1: {
		if(!mShow) {
			...
			{
#ifndef HYBRIDRENDER
				//if(mType[i]<0 && mPos[i].z<0) mPos[i].Set(-1000, -1000, -1000);
#else
				mPos[i] += Vector3DF(-25, 25, -25);
#endif
#ifdef HYBRID
				//if (mType[i] < 0)
				//	mPos[i].Set(-1000, -1000, -1000);
#endif
			}
		}
#ifndef CONSOLE
...
void FluidSystem::output_particles(int frame) {
	auto is_particles = [&](int idx) {
		return mType[idx] == 0 && mGridCell[idx] != GRID_UNDEF;
	};
	...
	char buf[256];
	sprintf(buf, "result/sph/particles_%d.txt", frame);
	FILE *fp = fopen(buf, "w");
	fprintf(fp, "%d\n", pnum);
	for (int i = 0; i < numPoints; i++) {
		if (is_particles(i))
			fprintf(fp, "%f %f %f %g\n", mPos[i].x, mPos[i].y, mPos[i].z, mMass[i]);
	}
	fclose(fp);
}
...
std::string FluidSystem::getModeStr ()
{
	...
	case RUN_CUDA_PBD:		sprintf ( buf, "SIMULATE CUDA PBD" );		break;
	};
...
//    Actual visocity of water = 0.001 Pa.s    // viscosity of water at 20 deg C.
	...
	for (int i = 0; i < TYPE_NUM; i++) {
		mpmass.tid[i] = 0.00020543;
		mprest_dens.tid[i] = 600;
	}

	// Load settings from XML (overwrite the above defaults)
	ParseXMLFluid ( "Fluid", 0, true );
}

int FluidSystem::ParseXMLFluid ( std::string name, int id, bool bStart )
{
	xml.setBase ( name, id );

	xml.assignValueD ( &m_DT, "DT" );
	xml.assignValueStr ( mSceneName, "Name" );
	if (bStart)	xml.assignValueD ( &m_Param[PNUM], "Num" ); // max number of particles
	xml.assignValueD ( &m_Param[PMODE], "Mode" ); // the simulation method
	xml.assignValueD ( &m_Param[PMASS], "Mass" ); // standard mass
	...
	xml.assignValueD ( &m_Param[PRESTDENSITY], "RestDensity" ); // standard density
	...
	xml.assignValueD ( &m_Param[PLAMBDA_EPS], "LambdaEpsi" );
	xml.assignValueD ( &m_Param[PVORT_EPS], "VortEpsi" );
	xml.assignValueD ( &m_Param[PSCENE_NUM], "SceneNum" ); // number of scenes

	xml.assignValueM ( &mpmass, "MassRatio" );
...
void FluidSystem::ParseXMLScene ( std::string name, int id )
{
	xml.setBase ( name, id );

	xml.assignValueM ( &malpha[id - 1][PMOLE_RATIO], "MoleRatio");
	xml.assignValueV3 ( &m_SceneVec[id-1][PINITMIN], "InitMin" );
	xml.assignValueV3 ( &m_SceneVec[id-1][PINITMAX], "InitMax" );
}
...
void FluidSystem::SetupExampleParams ( bool bStart )
{
	...
	//printf("sceneNum = %d \n", (int)m_Param[PSCENE_NUM]);
	for (int i = 1; i <= (int)m_Param[PSCENE_NUM]; i++){
		ParseXMLScene ( "Scene", i );
	}
}
...
#else
	nvImg img ( width, height, 3, 8 );			// allocates pixel memory
#endif
    FILE *fScreenshot;
	char fileName[64];
...
#ifdef HYBRID
void FluidSystem::UpdateFromSWE()
{
	int cpnum = GetSPHCurrentPnum();

	int pAdded = AddParticlesFromSWE(cpnum);

	UpdateSPHCurrentPnum(pAdded);

	if (pAdded > 0)
	{
		printf("Added %d particles to SPH\n", pAdded);
		extern bool bPause;
		//bPause = !bPause;
	}
...
void FluidSystem::AddInfluenceToSWE()
{
	PrepareLabelParticlesForSWE();

	CollectLabelParticlesSWE();
}

void FluidSystem::AddEmitFull(float spacing, int cat, float mass)
{
	int p;
	Vector3DF dir;
	Vector3DF pos;
	...
	for (int n = 0; n < m_Vec[PEMIT_RATE].y; n++) {
		...
		////
		if (mMaxPoints<maxAllowedPoints)
		{
			mMaxPoints++;
			p = AddParticle(0);
			*(mPos + p) = pos;
...
void FluidSystem::EmitParticlesFull(float spacing, int cat, float mass)
{
	mMaxPoints = GetSPHCurrentPnum();
	int currentPoints = mMaxPoints;

	float ss = 0.45f;
	float addMass = pow((ss / 0.87*0.005), 3.0) * 600;

	//float addMass = 0.05f;// *pow(0.005, 3.0);
	//float ss = pow(addMass / 0.35, 1.0 / 3.0) * 0.87;// / 0.005;
	m_Vec[PEMIT_RATE] = Vector3DF(10, 1000, 9);

	if (m_Vec[PEMIT_RATE].x > 0 && (m_Frame) % (int)m_Vec[PEMIT_RATE].x == 0 && m_Frame < 50) {

		m_Vec[PEMIT_SPREAD] = Vector3DF(0, 0, 0);
		m_Vec[PEMIT_ANG] = Vector3DF(-90, 180, 0);
		m_Vec[PEMIT_POS] = Vector3DF(20, 25, 25);
		AddEmitFull(ss, cat, addMass);

	}
	EmitUpdateCUDA(currentPoints, mMaxPoints);
	mNumPoints = currentPoints;
	printf("Emit %d\n", mMaxPoints - currentPoints);
}
...
void FluidSystem::EmitUpdateCUDA(int startnum, int endnum)
{
	...
	CopyEmitToCUDA((float*)mPos, (float*)mVel, (float*)mVelEval, (float*)mForce, mPressure, mDensity, mClusterCell, mGridNext, (char*)mClr, mType, mMass, mRestDens, mAlpha, mSWEvertex, startnum, numpoints);

	UpdateSPHCurrentPnum(numpoints);
}
#endif