...
:coefficient_matrix_(NULL),preconditioner_(NULL)
{
...
template <typename Scalar>
const GeneralizedMatrix<Scalar>* LinearSystem<Scalar>::preconditioner() const
{
    return preconditioner_;
}

template <typename Scalar>
GeneralizedMatrix<Scalar>* LinearSystem<Scalar>::preconditioner()
{
    return preconditioner_;
}

template <typename Scalar>
void LinearSystem<Scalar>::setPreconditioner(const GeneralizedMatrix<Scalar> &matrix)
{
    if(preconditioner_)
        delete preconditioner_;
...
template <typename Scalar>
void LinearSystem<Scalar>::preconditionerMultiply(const GeneralizedVector<Scalar> &x, GeneralizedVector<Scalar> &result) const
{
    if(preconditioner_)
    {
        const PlainGeneralizedVector<Scalar> &plain_x = dynamic_cast<const PlainGeneralizedVector<Scalar>&>(x);
...
template <typename Scalar>
void LinearSystem<Scalar>::computeJacobiPreconditioner()
