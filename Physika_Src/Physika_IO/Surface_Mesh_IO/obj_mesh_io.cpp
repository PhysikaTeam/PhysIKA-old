...
template <typename Scalar>
bool ObjMeshIO<Scalar>::saveToMesh2(const std::string & filename, const SurfaceMesh<Scalar> * mesh)
{
	string file_extension = FileUtilities::fileExtension(filename);
	if(file_extension.size() == 0)
	{
		std::cerr<<"No file extension found for the mesh2 file:"<<filename<<std::endl;
		return false;
	}
	if(file_extension != string(".mesh2"))
	{
