...
public:
    ...
    Vector(const Vector<Scalar,1>&) = default;
    ~Vector() = default;
    ...
    Vector<Scalar,1>& operator= (const Vector<Scalar,1> &) = default;

    bool operator== (const Vector<Scalar,1> &) const;
    bool operator!= (const Vector<Scalar,1> &) const;

    const Vector<Scalar,1> operator+ (Scalar) const;
    const Vector<Scalar,1> operator- (Scalar) const;
    const Vector<Scalar,1> operator* (Scalar) const;
    const Vector<Scalar,1> operator/ (Scalar) const;
    ...
    const Vector<Scalar, 1> operator - (void) const;
    ...
    const SquareMatrix<Scalar,1> outerProduct(const Vector<Scalar,1>&) const;

protected:

#ifdef PHYSIKA_USE_EIGEN_VECTOR
    Eigen::Matrix<Scalar,1,1> eigen_vector_1x_; //default: 0
...
inline const Vector<T,1> operator *(S scale, const Vector<T,1> &vec)
{
    return vec * scale;
}
