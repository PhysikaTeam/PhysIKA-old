#ifndef __SimulationDataDFSPH_K_h__
#define __SimulationDataDFSPH_K_h__
...
{	
	...
	*/
	class SimulationDataDFSPH_K
	{
		public:
			SimulationDataDFSPH_K();
			virtual ~SimulationDataDFSPH_K();
		...
		protected:	
			...
			// particles cluster
			std::vector<std::vector<unsigned int>> m_cluster;
		...
		public:
			...
			*/
			...
			//-------------
			FORCE_INLINE const unsigned int getCluster(const unsigned int fluidIndex, const unsigned int i) const
			{
				return m_cluster[fluidIndex][i];
			}

			FORCE_INLINE unsigned int& getCluster(const unsigned int fluidIndex, const unsigned int i)
			{
				return m_cluster[fluidIndex][i];
			}

			FORCE_INLINE void setCluster(const unsigned int fluidIndex, const unsigned int i, const unsigned int c)
			{
				m_cluster[fluidIndex][i] = c;
			}
			FORCE_INLINE std::vector<unsigned int>& getCluster(const unsigned int fluidIndex)
			{
				return m_cluster[fluidIndex];
			}
