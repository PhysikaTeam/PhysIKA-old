...
namespace SPH
{
	enum class SimulationMethods { WCSPH = 0, PCISPH, PBF, IISPH, DFSPH, PF, DFSPH_K, NumSimulationMethods };
	...
	*/
	...
	public:
		...
		static int ENUM_SIMULATION_DFSPH_K;
	...
	public:
		...
		//---------cfl最大速度----------
		FORCE_INLINE Real getMaxV() const { return m_maxV; }
		FORCE_INLINE void setMaxV(Real maxV) { m_maxV = maxV; }
		//---------聚类阈值速度----------
		FORCE_INLINE Real getThresholdV() const { return m_thresholdV; }
		FORCE_INLINE void setThresholdV(Real thresholdV) { m_thresholdV = thresholdV; }
		//---------主时间步长----------
		Real getMainTimeStep() const { return m_mainTimeStep; }
		void updateMainTimeStep(Real k1_maxV);
		...
		*/
		...
		{
			//2021/3/9
			if (m_neighborhoodSearch->is_active(pointSetIndex, neighborPointSetIndex)) {
