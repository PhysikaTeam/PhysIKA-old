...
void updateBunny(float, float, float);
...
static int bunnyDL = 0;

int genBunnyDispList()
{
		GLuint id = 0;
		id = glGenLists(1);
...
void useBunnyDL(double dx, double dy, double dz,
...
extern void projectOutside(double);
extern void exportCloth(const char *ipath, const char *opath);
extern void checkCollision();

void key9()
{
	projectOutside(gDepOff);
...
void updateLight()
{
	...
	updateBunny(lightpos[0], lightpos[1], lightpos[2]);
}
...
//find matching nodes for handles
int main(int argc, char **argv)
{
	if (argc != 5) {
		printf("usage: %s cloth.obj body.obj nodes.txt out.txt\n", argv[0]);
		return 0;
	}

	initModel(argv[1], argv[2]);
	findMatchNodes(argv[3], argv[4]);
...
//int main(int argc, char **argv)
int main_self_cd_cmd(int argc, char **argv)
{
	if (argc < 2) {
		printf("usage: %s input.obj [out.txt]\n", argv[0]);
		return -1;
	}

	initModel(argv[1]);
	checkSelfCPU_Refit((argc == 3) ? argv[2] : NULL);
	return 0;
}
