...
{
    //std::cout << "is_valid_the_id :" << the_id << std::endl;
    return the_id != ncBad;
}
    ...
    size_t* bufrsizeptr, size_t initialsize, FileFormat fformat)
{
    ...
    //std::cout << "fformat: " << fformat << std::endl;
    ...
    case ReadOnly:
        ...
        in_define_mode = 0;
        //std::cout << "the_id: " << the_id << std::endl; 
        break;
    case New:
        mode |= NC_NOCLOBBER;
        /*FALLTHRU*/
...
NcValues* NcTypedComponent::get_space(long numVals) const
{
	...
	std::cout << "get_space()_num_vals() : " << numVals << std::endl;
...
NcValues* NcVar::get_rec(NcDim* rdim, long slice)
{
	...
	std::cout << "rec_size(rdim) :" << rec_size(rdim) << std::endl;
