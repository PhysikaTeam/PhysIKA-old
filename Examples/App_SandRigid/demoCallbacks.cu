...
namespace PhysIKA
{
    ...
    __global__ void PGCallback_generate(
        ...
        int startid,
        double m0,
        double gxmin,double gxmax, double gzmin, double gzmax
    )
    {
        int tid = threadIdx.x + blockDim.x*blockIdx.x;
        if (tid + startid >= parPos.size()) return;
    ...
    void ParticleGenerationCallback::init(float xmin, float xmax, float zmin, float zmax,
        float m0, float rate, int maxNum, long seed)
    {
        gxMin = xmin;
        gxMax = xmax;
        gzMin = zmin;
        ...
        cuExecute(maxNum, PGCallback_init,
            devStates, maxNum, seed);
    }
    ...
    void ParticleGenerationCallback::handle(ParticleSandRigidInteraction * interactNode, float dt)
    {
        ...
        cuExecute(newParNum, PGCallback_generate,
            devPos, devVel, devMass, devType,
            devStates,
            startid, (double)particelMass,
