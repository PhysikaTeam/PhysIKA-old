	...
	*/
	...
	class ParticleData
	{
		private:
			...
			std::vector<Real> m_vapor; //vapor含量
			std::vector<Real> m_cloud;  //water含量
		...
		public:
			...
			FORCE_INLINE void addVertex(const Vector3r &vertex)
			{
				...
				// for cloud's properties
				m_temperature.push_back(16.85); //16.85 equals 290K (绝对温度)
			...
			*/
			FORCE_INLINE void deleteVertex(const unsigned int i)
			{
				...
				// for cloud's properties
				m_temperature.erase(m_temperature.begin() + i); //16.85 equals 290K (绝对温度)
