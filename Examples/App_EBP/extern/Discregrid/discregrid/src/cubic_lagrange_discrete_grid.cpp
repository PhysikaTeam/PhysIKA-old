...
zValue(Vector3d const& x, double invCellSize)
{
	...
	{
		if (x[i] >= 0.0) key[i] = static_cast<int>(invCellSize * x[i]);
		else key[i] = static_cast<int>(invCellSize * x[i]) - 1;
	}
...
void
CubicLagrangeDiscreteGrid::save(std::string const& filename) const
{
...
void
CubicLagrangeDiscreteGrid::load(std::string const& filename)
{
	...
	for (int l = 0; l < static_cast<int>(n_nodes); ++l)
	{
		...
		if (++counter == n_nodes || duration_cast<milliseconds>(high_resolution_clock::now() - t0).count() > 10u)
		{
			std::async(std::launch::async, [&]()
			{
				...
				std::cout << "\r" << "Construction " << std::setw(20)
...
void
CubicLagrangeDiscreteGrid::reduceField(unsigned int field_id, Predicate pred)
{
...
void
CubicLagrangeDiscreteGrid::forEachCell(unsigned int field_id,
