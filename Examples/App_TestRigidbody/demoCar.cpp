...
void DemoCar2::addCar(std::shared_ptr<PBDCar> car, Vector3f pos,
...
void DemoTankCar::build(bool useGPU)
{
       ...
       this->addCar(m_car, Vector3f(0.1, /*-0.15*/ 0.40, 0.1),
                    1, 4 | 8, 2, 4 | 8);
...
void DemoTankCar::addCar(std::shared_ptr<MultiWheelCar<4>> car, Vector3f pos,
                         int chassisGroup, int chassisMask, int wheelGroup, int wheelMask)
{
       car->m_rigidSolver = m_groundRigidInteractor->getSolver();
       //m_car->m_rigidSolver->setUseGPU(useGPU);
