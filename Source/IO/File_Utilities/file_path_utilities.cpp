...
#include "IO/File_Utilities/file_path_utilities.h"
using std::string;
...
string dirName(const string& path)
{
    ...
    if (pos1 != string::npos && pos2 != string::npos && pos1 < pos2)
        pos1 = pos2;  //and store it in variable pos1
    else if (pos1 == string::npos && pos2 == string::npos)
        return string(".");
    else if (pos1 == string::npos)
        pos1 = pos2;
    if (path[pos1] == '\\')
        return path.substr(0, pos1 - 1);  //if pos1 stores char '\', we should return path.substr(0,pos1-1) because '\'always apear in pairs
