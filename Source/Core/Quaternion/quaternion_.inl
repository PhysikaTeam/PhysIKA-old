...
template <typename Real>
COMM_FUNC void Quaternion<Real>::set(const Vector<Real, 3>& vec3, Real scale)
{
    w_ = scale;
    x_ = vec3[0];
    y_ = vec3[1];
...
template <typename Real>
COMM_FUNC void Quaternion<Real>::set(Real scale, const Vector<Real, 3>& vec3)
{
    w_ = scale;
    x_ = vec3[0];
    y_ = vec3[1];
...
template <typename Real>
COMM_FUNC void Quaternion<Real>::rotateVector(Vector<Real, 3>& v)
{
    Real xlen = v.norm();
    if (xlen == 0.0f)
        return;
...
COMM_FUNC SquareMatrix<Real, 4> Quaternion<Real>::get4x4Matrix() const
{
    ...
    entries[14] = 0;
    entries[15] = 1;
    return SquareMatrix<Real, 4>(entries[0], entries[1], entries[2], entries[3], entries[4], entries[5], entries[6], entries[7], entries[8], entries[9], entries[10], entries[11], entries[12], entries[13], entries[14], entries[15]);
}
...
template <typename Real>
COMM_FUNC void Quaternion<Real>::toRotationAxis(Real& rot, Vector<Real, 3>& axis) const
{
