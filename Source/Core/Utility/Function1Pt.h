...
namespace PhysIKA
{
	...
	{ 
		...
		void copy(Array<T, dType1>& arr1, Array<T, dType2>& arr2)
		{
			...
			if (arr1.isGPU() && arr2.isGPU()) { cuSafeCall(cudaMemcpy(arr1.begin(), arr2.begin(), totalNum * sizeof(T), cudaMemcpyDeviceToDevice)); }
			else if (arr1.isCPU() && arr2.isGPU()) { cuSafeCall(cudaMemcpy(arr1.begin(), arr2.begin(), totalNum * sizeof(T), cudaMemcpyDeviceToHost)); }
			else if (arr1.isGPU() && arr2.isCPU()) { cuSafeCall(cudaMemcpy(arr1.begin(), arr2.begin(), totalNum * sizeof(T), cudaMemcpyHostToDevice)); }
		...
		void copy(ArrayPitch2D<T, dType1>& g1, ArrayPitch2D<T, dType2>& g2)
		{
			...
			int width = g1.Nx() * sizeof(T);
			int height = g1.Ny();
			if (g1.IsGPU() && g2.IsGPU()) { cuSafeCall(cudaMemcpy2D(g1.GetDataPtr(), g1.Pitch() * sizeof(T), g2.GetDataPtr(), g2.Pitch() * sizeof(T), width, height, cudaMemcpyDeviceToDevice)); }
			else if (g1.IsCPU() && g2.IsGPU()) { cuSafeCall(cudaMemcpy2D(g1.GetDataPtr(), g1.Pitch() * sizeof(T), g2.GetDataPtr(), g2.Pitch() * sizeof(T), width, height, cudaMemcpyDeviceToHost)); }
			else if (g1.IsGPU() && g2.IsCPU()) { cuSafeCall(cudaMemcpy2D(g1.GetDataPtr(), g1.Pitch() * sizeof(T), g2.GetDataPtr(), g2.Pitch() * sizeof(T), width, height, cudaMemcpyHostToDevice)); }
			else if (g1.IsCPU() && g2.IsCPU()) { cuSafeCall(cudaMemcpy2D(g1.GetDataPtr(), g1.Pitch() * sizeof(T), g2.GetDataPtr(), g2.Pitch() * sizeof(T), width, height, cudaMemcpyHostToHost)); }
		}
		...
		void copy(Array3D<T, dType1>& g1, Array3D<T, dType1>& g2)
		{
			...
			if (g1.IsGPU() && g2.IsGPU()) { cuSafeCall(cudaMemcpy(g1.GetDataPtr(), g2.GetDataPtr(), totalNum * sizeof(T), cudaMemcpyDeviceToDevice)); }
			else if (g1.IsCPU() && g2.IsGPU()) { cuSafeCall(cudaMemcpy(g1.GetDataPtr(), g2.GetDataPtr(), totalNum * sizeof(T), cudaMemcpyDeviceToHost)); }
			else if (g1.IsGPU() && g2.IsCPU()) { cuSafeCall(cudaMemcpy(g1.GetDataPtr(), g2.GetDataPtr(), totalNum * sizeof(T), cudaMemcpyHostToDevice)); }
