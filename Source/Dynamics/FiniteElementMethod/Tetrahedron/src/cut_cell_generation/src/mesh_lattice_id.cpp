...
size_t Mesh::get_axis_grid(
  size_t axis_itr, const vector<size_t> &sequence,
...
size_t Mesh::get_line_vert_axis_grid_id(
  ...
  const vector<double> &grid_axis, vector<size_t> &grid_id)
{
...
size_t Mesh::set_projection_edge_vert_axis_grid_id(
  ...
  const vector<double> &grid_axis, size_t id_grid_line,
  vector<size_t> &grid_id, const Vector2d v_projection[2])
{
  const MatrixXd &aabb_m = get_aabb();
