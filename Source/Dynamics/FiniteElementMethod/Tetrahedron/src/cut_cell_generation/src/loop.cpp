...
int Loop::set_grid_edge()
{
  ...
  while (!polygon_cutted_.empty())
  {
    ...
    polygon_.insert(polygon_.end(),
                    polygon_origin_cutted.begin(), polygon_origin_cutted.end());
  }
...
int Loop::set_inner_polygon_lattice_id()
{
  for (const auto &poly : polygon_)
  {
    ...
    reverse_copy(poly.begin(), poly.end(),
                 back_inserter(poly_reverse));
...
int Loop::set_inner_polygon(
  size_t itr, size_t i, vector<vector<size_t>> &polygon_origin)
{
...
int Loop::get_section_grid_edge(
  ...
  size_t t, const vector<size_t> &id_tri, const vector<size_t> &id_line_v,
  const vector<Edge> &e_line, vector<Edge> &edge_grid,
  vector<size_t> &v_section_id)
{
  size_t axis = ring_->axis_;
  ...
  get_grid_aabb(id_tri[t], id_tri[t+1],
                axis_projection, id_lower, id_upper);
...
int Loop::get_grid_aabb(
  size_t id_tri_1, size_t id_tri_2,
...
int Loop::remove_duplicate_vert(
  size_t itr, vector<size_t> &vert_line, vector<Edge> &edge_line)
{
...
int Loop::sort_one_line(size_t itr, size_t i)
{
  ...
  sort(ordered_seq.begin(), ordered_seq.end(),
       [this, &vert_line, &edge_line, itr, id_grid](size_t order_1, size_t order_2)
       {
         if (order_1 == order_2)
           return false;
...
int Loop::update_sorted_vert(
  size_t itr, size_t id_grid, const vector<size_t> &ordered_seq,
  const vector<size_t> &vert_line, const vector<Edge> &edge_line)
{
  vector<size_t> vert_sorted_one_line;
  vector<Edge> edge_sorted_one_line;
  transform(ordered_seq.begin(), ordered_seq.end(),
            back_inserter(vert_sorted_one_line),
            [&vert_line](auto itr)
            {
              return vert_line[itr];
            });
  transform(ordered_seq.begin(), ordered_seq.end(),
            back_inserter(edge_sorted_one_line),
            [&edge_line](auto itr)
            {
...
bool Loop::is_vert_front(size_t itr, size_t id_grid,
