...
vector<Ring> Mesh::cut_mesh_on_axis_at_p(size_t axis, size_t p)
{
  ...
  for (size_t id_tri = 0; id_tri < num_tri; ++id_tri)
  {
    ...
    cut_one_tri(aabb(axis, 0), aabb(axis, 1), axis, p, p_grid, id_tri,
                ring_lower, ring_upper, is_coplanar);
  }
  ...
  for (size_t itr = 1; itr <= 2; ++itr)
  {
    ...
    while (id_grid_start < id_grid_end + 1)
    {
      ...
      int is_inside = is_vert_inside_triangle(
        id_tri, axis_projection,
        axis_edge, id_axis_edge_grid, axis_cut, id_grid_start,
        v1_e, v2_e);
      
      if (is_inside != -1)
      {
        Vert *vert_ptr = new VertLine(id_tri, axis_edge, id_axis_edge_grid,
                                      axis_cut, id_grid_start);
...
int Mesh::add_triangle_parallel_grid_edge(
  size_t id_tri, size_t itr, const Edge &e)
{
  Triangle &tri = table_triangle_.at(id_tri);
...
int Mesh::add_patch(
  const Vector3st &id_lattice, const pair<PatchType, size_t> &patch)
{
  for (size_t axis = 0; axis < 3; ++axis)
    assert(id_lattice[axis] != numeric_limits<size_t>::max());
