...
private:
  int remove_duplicate_vert(
    size_t itr, std::vector<size_t> &vert_line, std::vector<Edge> &edge_line);
...
private:
  int get_one_line_invalid_edge(
    const std::pair<size_t, std::vector<Edge>> &one_line, size_t itr);
  int set_invalid_edge();
  int get_next_invalid_edge(
    const Edge &e, const Edge &next_e, size_t itr,
    size_t grid_cutting, bool is_next_edge);

  size_t next_vert_of_v_on_polygon(
    size_t id_v, const std::vector<size_t> &v_on_line,
    const std::vector<size_t> &poly_vert);

  bool is_vertex_on_same_boundary(
    size_t id_prev_v, size_t id_end_v,
    const std::vector<std::vector<size_t>> &polygon_origin);

  int connect_boundary(
    const std::vector<size_t> &v_section_id, size_t id_prev_v, size_t id_end_v,
    std::vector<std::vector<size_t>> &polygon_origin);

  std::vector<size_t> get_vertex_sequence(
    size_t v1, size_t v2, const std::vector<size_t> &poly);

  int set_inner_polygon(
    size_t itr, size_t i, std::vector<std::vector<size_t>> &polygon_origin);

  int set_cutted_polygon(
    size_t itr, size_t i, std::vector<std::vector<size_t>> &polygon_origin);

  int get_inner_polygon(
    size_t itr, const std::vector<size_t> &id_line_v,
    const std::vector<size_t> &v_section_id, size_t t,
    std::vector<std::vector<size_t>> &polygon_origin);
  ...
  std::vector<size_t> get_polygon_boundary_line_vert(
    ...
    const std::vector<std::vector<size_t>> &polygon_origin);
  int resort_boundary(const std::vector<size_t> &poly, size_t id_v,
                            std::vector<size_t> &poly_new);
  std::vector<size_t> get_polygon(
    size_t id_v, std::vector<std::vector<size_t>> &polygon_origin);
  

private:
  int sort_one_line(size_t itr, size_t i);
  int update_sorted_vert(
    size_t itr, size_t id_grid, const std::vector<size_t> &ordered_seq,
    const std::vector<size_t> &vert_line, const std::vector<Edge> &edge_line);
  bool is_vert_front(size_t itr, size_t id_grid,
                     size_t id_v1, const Edge &e1,
                     size_t id_v2, const Edge &e2);
  ...
  std::vector<int> get_vert_inside(
    size_t itr, size_t id_grid, double *p, const Edge &e);

  int set_axis_grid_edge_on(size_t itr, size_t i);
  int get_section_grid_edge(
    size_t itr, size_t id_grid,
    size_t t, const std::vector<size_t> &id_tri,
    const std::vector<size_t> &id_line_v,
    const std::vector<Edge> &e_line, std::vector<Edge> &edge_grid,
    std::vector<size_t> &v_section_id);

  int get_grid_aabb(size_t id_tri_1, size_t id_tri_2,
                    size_t axis_projection, size_t &id_lower, size_t &id_upper);
