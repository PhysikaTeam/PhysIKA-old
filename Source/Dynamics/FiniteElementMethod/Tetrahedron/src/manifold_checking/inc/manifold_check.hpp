#ifndef __MANIFOLD_CHECK_HPP__
#define __MANIFOLD_CHECK_HPP__
...
bool is_triangle_triangle_self_intersect(const Eigen::Matrix<T, 3, 3> &tri_a, const Eigen::Matrix<T, 3, 3> &tri_b)
{
  ...
  for (size_t i = 0; i < 3; ++i)
  {
    ...
    position_a2b[i] = (a2b == 0 ? 0 : a2b > 0 ? 1 : -1);
    position_b2a[i] = (b2a == 0 ? 0 : b2a > 0 ? 1 : -1);
  }
...
int is_common_condition(const Eigen::Matrix<T, 3, 3> &tri_a, const Eigen::Matrix<T, 3, 3> &tri_b, const std::array<int, 3> &position_a2b, const std::array<int, 3> &position_b2a)
{
  ...
  int num_coincidence =
    accumulate(idx.begin(), idx.end(), 0,
               [&tri_a, &tri_b] (int b, int v) -> int
               {
  ...
  {
    int sum_coplanar = accumulate(position_a2b.begin(), position_a2b.end(), 0,
                                  [] (int b, int v) -> int
                                  {
                                    return b + (v == 0);
                                  });
...
{
  int sum_coplanar = accumulate(position.begin(), position.end(), 0,
                                [] (int b, int v) -> int
                                {
                                  return b + (v == 0);
                                });
...
bool is_triangle_triangle_self_intersect(const Eigen::Matrix<T, 3, 3> &tri_a, const Eigen::Matrix<T, 3, 3> &tri_b, const std::array<int, 3> &position_a2b, const std::array<int, 3> &position_b2a)
{
  ...
  if (sum_coplanar == 3)
  {
    int sum_dual_coplanar = accumulate(position_b2a.begin(), position_b2a.end(), 0,
                                       [] (int b, int v) -> int
                                       {
                                         return b + (v == 0);
                                       });
...
bool is_vert_triangle_intersect(const Eigen::Matrix<T, 3, 1> &v, const Eigen::Matrix<T, 3, 1> &p, const Eigen::Matrix<T, 3, 3> &tri, bool coplanar_v_triangle)
{
  ...
  for (size_t i = 0; i < 3; ++i)
  {
    ...
    position[i] = (dis == 0 ? 0 : dis > 0 ? 1 : -1);
  }

  int sum_tet_plane = accumulate(position.begin(), position.end(), 0,
                              [] (int b, int p) -> int
                              {
                                return b + (p == 0);
                              });
