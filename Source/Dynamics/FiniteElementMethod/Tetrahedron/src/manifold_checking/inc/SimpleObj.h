#ifndef __SIMPLEOBJ_H__
#define __SIMPLEOBJ_H__
...
public:
	...
	void smooth(const bool anisotropic=false, const double step=1e-6, const double smooth_weight=0.5, 
		const double threshold=30, const bool verbose=true);
	void continuously_smooth(const int n_iter, const bool anisotropic=false, const double step=1e-6, const double smooth_weight=0.5, 
		const double threshold=30, const bool verbose=true); // Pre: compute_all_curvatures()
