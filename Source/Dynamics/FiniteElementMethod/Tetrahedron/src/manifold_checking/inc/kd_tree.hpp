...
Node<T>* KdTree<T>::setup_tree(const std::vector<size_t> &table_tri_id, size_t axis)
{
  ...
  sort(table_index.begin(), table_index.end(),
       [&table_c, axis] (size_t l, size_t r) -> bool
       {
         return table_c[l](axis) < table_c[r](axis);
       });

  size_t mid = table_c.size() / 2;
  vector<size_t> table_lower_index
    (table_index.begin(), table_index.begin() + mid);
  vector<size_t> table_upper_index
    (table_index.begin() + mid, table_index.end());
  vector<size_t> table_lower_tri;
  vector<size_t> table_upper_tri;
  transform(table_lower_index.begin(), table_lower_index.end(),
            back_inserter(table_lower_tri),
            [&table_tri_id] (size_t i)
            {
              return table_tri_id.at(i);
            });
  transform(table_upper_index.begin(), table_upper_index.end(),
            back_inserter(table_upper_tri),
            [&table_tri_id] (size_t i)
            {
              return table_tri_id.at(i);
            });
...
std::vector<Eigen::Matrix<T, 3, 3>> KdTree<T>::get_neigh_tri(size_t id_tri) const
{
  ...
  transform(table_neigh_tri_id.begin(), table_neigh_tri_id.end(),
            back_inserter(table_neigh_tri),
            [this] (size_t id_tri)
            {
