...
void SimpleObj::set_obj(const string &obj_file) {
	...
	if (v_num > 0) v_mat = MatrixXd::Zero(3, v_num);
    if (vn_num > 0) vn_mat = MatrixXd::Zero(3, vn_num);
    if (vt_num > 0) vt_mat = MatrixXd::Zero(3, vt_num);
	...
	while (getline(input, line)) {
		...
		if (word == "usemtl" || word == "mtllib" || word == "g" || word == "G" || word == "s" || word == "S" || word == "o" || word == "O" || word == "L" || word == "l") {
			...
			while (true) {
				...
				if (f_v_idx >= 3) report_err("Error: In OBJ file, some faces are not triangle");
				get_from_face_data(face_data, f_val, ft_val, fn_val);
				f_mat(f_v_idx, f_idx) = f_val - 1;
				if (ft_val > 0) ft_mat(f_v_idx, f_idx) = ft_val - 1;
				else ft_mat(f_v_idx, f_idx) = -1;
				if (fn_val > 0) fn_mat(f_v_idx, f_idx) = fn_val - 1;
				else fn_mat(f_v_idx, f_idx) = -1;
				f_v_idx++;
			}
			if (f_v_idx != 3) report_err("Error: In OBJ file, some faces are not triangle");
			f_idx++;
...
void SimpleObj::change_axis(const string x, const string y, const string z) {
	auto decode_target_axis = [=] (const string axis, const double x, const double y, const double z) {
		if (axis == "x") return x;
		else if (axis == "-x") return -x;
		else if (axis == "y") return y;
		else if (axis == "-y") return -y;
		else if (axis == "z") return z;
		else if (axis == "-z") return -z;
		else return 0.0;
	};
	for (int i=0; i<v_num; i++) {
...
void SimpleObj::clean(const double threshold, const bool verbose) {
	...
	if (verbose) cout << "[INFO] Original f_num: " << f_num << "; After cleaning: " << cnt << endl;
	f_num = cnt;
...
void SimpleObj::build_one_ring_faces_set(const bool verbose) {
	...
	if (verbose) cout << "[INFO] build_one_ring_faces_set()" << endl;
}
...
void SimpleObj::build_one_ring_vertices_dict(const bool verbose) {
	...
	if (verbose) cout << "[INFO] build_one_ring_vertices_dict()" << endl;
}
...
void SimpleObj::build_is_boundary(const bool verbose) {
	...
	for (size_t i=0; i<v_num; i++) {
		...
		if (is_boundary[i] > 0) boundary_num++;
		if (is_boundary[i] < 0 && verbose) cerr << "[WARNING]: Exists vertex has more one-ring faces than one-ring vertices: vertex id: " << i << endl;
	}
	if (verbose) cout << "[INFO] build_is_boundary()" << endl;
}
...
void SimpleObj::compute_angles(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_angles()" << endl;
}
...
void SimpleObj::compute_face_voronoi_areas(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_face_voronoi_areas()" << endl;
}
...
void SimpleObj::compute_vertex_voronoi_areas(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_vertex_voronoi_areas()" << endl;
}
...
double SimpleObj::compute_directed_volume(const string& type, const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_directed_volume(): volume " << volume << endl;
	return volume;
}
...
double SimpleObj::sum_cot_angle(const size_t idx, const size_t neighbor) {
	if (!has_built_one_ring_vertices_dict) build_one_ring_vertices_dict();
	if (!has_computed_angles) compute_angles();
...
void SimpleObj::compute_normals_KHs(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_normals_KHs()" << endl;
}

void SimpleObj::correct_normal_direction(const size_t idx, Vector3d &normal) {
	if (!has_built_one_ring_faces_set) build_one_ring_faces_set();
...
void SimpleObj::compute_KGs(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_KGs()" << endl;
}
...
void SimpleObj::compute_Kmaxs_Kmins(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_Kmaxs_Kmins()" << endl;
}
...
void SimpleObj::compute_all_curvatures(const bool verbose) {
	...
	if (verbose) cout << "[INFO] compute_all_curvatures()" << endl;
}
...
void SimpleObj::naive_convex(const bool verbose) {
	...
	if (verbose) cout << "[INFO] naive_convex()" << endl;
}
...
void SimpleObj::smooth(const bool anisotropic, const double step, const double smooth_weight, 
	const double threshold, const bool verbose) {
	reset_v_min_max();
	if (anisotropic) {
	...
	if (verbose) cout << "[INFO] smooth()" << endl;
}

void SimpleObj::continuously_smooth(const int n_iter, const bool anisotropic, 
	const double step, const double smooth_weight, const double threshold, 
	const bool verbose) {

	for (int i=0; i<n_iter; i++) {
		if (verbose) cout << "[INFO] continuously_smooth() iter: " << i << endl;
		compute_all_curvatures(verbose);
		smooth(anisotropic, step, smooth_weight, threshold, verbose);
	}
	if (verbose) cout << "[INFO] continuously_smooth()" << endl;
}
...
void SimpleObj::write_attrs_to_vtk(const string& save_file, const bool verbose) {
	...
	output.close();
	if (verbose) cout << "[INFO] write_attrs_to_vtk()" << endl;
}
