...
template<typename T>
MassSpringObj<T>::MassSpringObj(const char *path, T stiffness) : stiffness_(stiffness)
{
                          ...
                          std::shared_ptr<PhysIKA::dat_str_core<T,3>>& data) const
{
  ...
  for (const auto &e : edge_length_)
  {
    ...
    int v2 = e.first[1];
    T l0 = e.second;
    T v[6] = {x[3*v1], x[3*v1+1], x[3*v1+2],
              x[3*v2], x[3*v2+1], x[3*v2+2]};
