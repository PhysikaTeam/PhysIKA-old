...
{
public:
  semi_implicit(T h, const Eigen::Matrix<T, -1, 1> &m, const Eigen::Matrix<T, -1, 1> &x) : mass_(m), x_(x), h_(h)
    {
  ...
  Eigen::Matrix<T, -1, 1> get_velocity() const { return v_; }
  Eigen::Matrix<T, -1, 1> get_position() const { return x_; }
  Eigen::Matrix<T, -1, 1> get_mass() const { return mass_; }
