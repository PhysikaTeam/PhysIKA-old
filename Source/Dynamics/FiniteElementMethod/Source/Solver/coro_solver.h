...
class gmg_coro_solver: public newton_base<T, 3>{
 public:
  gmg_coro_solver(const std::shared_ptr<Problem<T, 3>>& pb,
      ...
      :newton_base<T, 3>(pb, max_iter, tol, line_search, hes_is_constant, nullptr, dat_str),
    pd_fac_(make_shared<gmg_coro_precond_fac<T>>(fac_hes_rest, elas_intf_ptr)), cg_tol_(cg_tol), hes_rest_(hes_rest){}
...
e U = \Lambda_tilde and \Lambda is the eigenavlues of K(x).
  ...
  */
  int test_spectrum(const SPM_R<T>& A, const size_t band, const size_t num_band,
                    MAT<T>& eig_vec)const{
...
class amg_coro_solver:public newton_base<T, 3>{
 public:
  amg_coro_solver(const std::shared_ptr<Problem<T, 3>>& pb,
                  const size_t max_iter, const T tol,
                  const bool line_search,const bool hes_is_constant,
                  const SPM<T>& hes_rest,const std::shared_ptr<ldlt_type<T>>& fac_hes_rest,
                  const T cg_tol = 1e-3, 
                  std::shared_ptr<dat_str_core<T, 3>> dat_str = nullptr)
      :newton_base<T, 3>(pb, max_iter, tol, line_search, hes_is_constant, nullptr, dat_str),
      pd_fac_(make_shared<amg_coro_precond_fac<T>>(fac_hes_rest, hes_rest)), cg_tol_(cg_tol){}
