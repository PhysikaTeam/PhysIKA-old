...
#define init_array_on_device(var, num, type) cudaMalloc((void**)& var, sizeof(type) * num); \
  cudaMemset(var, 0, num * sizeof(type)); \

template<typename T>
void GPU_PCG<T>::initialize(int Ntemp, int NNZtemp, T* Atemp, T* Btemp, int* IAtemp, int* JAtemp, constuctor_type TYPE) { //IAtemp 最后一项必须为总非零点数目
...
template<typename T>
GPU_PCG<T>::GPU_PCG(int Ntemp, int NNZtemp, T* Atemp, T* Btemp, int* IAtemp, int* JAtemp, constuctor_type TYPE) { //IAtemp 最后一项必须为总非零点数目
  initialize(Ntemp, NNZtemp, Atemp, Btemp, IAtemp, JAtemp, TYPE);
}
template<typename T>
GPU_PCG<T>::GPU_PCG(int Ntemp, int NNZtemp, const T* Atemp, const T* Btemp, const int* IAtemp, const int* JAtemp, constuctor_type TYPE) { //IAtemp 最后一项必须为总非零点数目
