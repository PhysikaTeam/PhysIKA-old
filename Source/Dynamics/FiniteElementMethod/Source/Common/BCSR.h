...
namespace PhysIKA {
  ...
  public:
    BCSR() : rowsb_(0), colsb_(0), rows_(0), cols_(0), size_(0) {}
    BCSR(int rowsb, int colsb)
        : rowsb_(rowsb), colsb_(colsb), rows_(rowsb * block_size),
          cols_(colsb * block_size), size_(cols_ * rows_) {}
  ...
  public:
    ...
    inline size_t rowsb() const { return rowsb_; }
    inline size_t colsb() const { return colsb_; }

    inline VEC_MAT<ele_type> get_value() const {return value_;}
    inline std::vector<size_t> get_offset()const{return offset_;}
    inline std::vector<size_t> get_index()const{return index_;}
    // operators override.
  public:
...
namespace PhysIKA {
  ...
  void BCSR<T, block_size>::setFromEigenMatrix(
    ...
    // very simple version.
    std::map<
        size_t, ele_type, std::less<size_t>,
