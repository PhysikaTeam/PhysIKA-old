...
class MyShitSolve{
 private:
  ...
  SPM_R<T> A_shift_;
 public:
  MyShitSolve(const SPM_R<T>&A):A_(A), I_(MAT<T>::Identity(A.rows(), A.cols()).sparseView()){}
  int rows() {return A_.rows();}
  int cols() {return A_.cols();}
  void set_shift(T sigma){sigma_ = sigma;    A_shift_ = A_ - sigma_ * I_;}
...
int eig_jac(const mat3<T>& mat_A, mat3<T>& eig_vec, Eigen::Matrix<T, 3, 1>& eig_val){
  ...
  auto get_Q_tan = [](const size_t& m, const size_t& n, const T& tan_theta)->mat3<T>{
    ...
    Q(m, m) = cos_theta;
    Q(n ,n) = cos_theta;
    Q(m, n) = -tan_theta * Q(m, m); Q(n, m) = -Q(m, n);
    return Q;
  };
  ...
  while(fabs(get_max_off_diag(An, m, n)) > 1e-8){
    ...
    cout << "An " << endl << An <<endl<< " Q " << endl << Q << endl << An * Q << endl << Q.transpose() * An * Q << endl;;
    An = Q.transpose() * An * Q;
    eig_vec = eig_vec * Q;
  };
  eig_val = An.diagonal();
