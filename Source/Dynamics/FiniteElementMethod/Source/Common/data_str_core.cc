...
dat_str_core<T, dim_>::dat_str_core(const size_t& dof, const bool hes_is_const):dof_(dof), val_(0), gra_(dof * dim_), hes_(dof * dim_, dof * dim_), all_one_(Matrix<T, Dynamic, 1>::Ones(dof * dim_)), hes_is_const_(hes_is_const), whole_dim_(dim_ * dof){
  set_zero();
}
