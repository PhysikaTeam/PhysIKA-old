...
struct spectra_params;

template <>
struct spectra_params<double>
{
    size_t maxits{ 1000 };
    double tol{ 1e-10 };
...
int solve_impl(Solver& slv, Eigen::Matrix<Real, -1, -1>* basis, Eigen::Matrix<Real, -1, 1>* lambda, Spectra::SortRule selection, Spectra::SortRule sorting, size_t maxits, Real tol)
{
    slv.init();
...
int general_eigen_value_decomposition(
    ...
    const Eigen::SparseMatrix<Real, Eigen::ColMajor>& B,
    int                                               num,
    ...
    const spectra_params<Real>&                       prm    = {})
{
    ...
    if (rule >= Spectra::SortRule::SmallestMagn && rule <= Spectra::SortRule::SmallestAlge)
    {
        try
        {
            using OpType = Spectra::SymShiftInvert<Real, Eigen::Sparse, Eigen::Sparse, Eigen::Lower, Eigen::Lower, Options, Eigen::ColMajor>;
            using BOpType =
            ...
            int ret = solve_impl(slv, basis, lambda, (Spectra::SortRule)(( int )rule - 4), rule, prm.maxits, prm.tol);
...
template float  k_consistent(const Eigen::SparseMatrix<float, Eigen::RowMajor>& Kref,
                             ...
                             const Eigen::Matrix<float, -1, 1>&                 Mdest,
                             int                                                start,
                             int                                                num);
template double k_consistent(const Eigen::SparseMatrix<double, Eigen::RowMajor>& Kref,
                             ...
                             const Eigen::Matrix<double, -1, 1>&                 Mdest,
                             int                                                 start,
                             int                                                 num);
