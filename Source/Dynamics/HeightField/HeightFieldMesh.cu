...
namespace PhysIKA
{
		...
		)
	{
		...
		if (tidx >= (hfield.Nx()) || tidy >= (hfield.Ny())) return;
	...
	)
	{
		int tidx = threadIdx.x + blockIdx.x * blockDim.x;
		int tidy = threadIdx.y + blockIdx.y * blockDim.y;
		if (tidx >= (hfield.Nx() - 1) || tidy >= (hfield.Ny() - 1)) return;
	...
	void HeightFieldMesh::generate(std::shared_ptr<TriangleSet<DataType3f>> triset, DeviceHeightField1d & hfield)
	{
		...
		cuExecute2D(gsizeVer, HFMesh_generateVertices,
			*pvertices, hfield
		);

		cuExecute2D(gsizeTri, HFMesh_generateTriangle,
			*ptriangles, hfield
		);
