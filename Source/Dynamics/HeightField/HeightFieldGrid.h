...
namespace PhysIKA
{
	...
	public:
		...
		COMM_FUNC inline int Nz() { return Ny(); }
		

		COMM_FUNC inline TReal getDx() { return m_dx; }
		COMM_FUNC inline TReal getDz() { return m_dz; }

		COMM_FUNC inline Coord getOrigin() { return origin; }
		...
		*/
		COMM_FUNC inline int2 gridIndex(Coord pos)
		{
			...
			res.x = min(res.x, Nx() - 1);	res.x = max(res.x, 0);
			res.y = min(res.y, Ny() - 1);	res.y = max(res.y, 0);
			return res;
		}
		...
		*/
		COMM_FUNC inline bool onTopofGrid(int i, int j, 
			...
			Coord& res)
		{
			...
			res[0] = detv; res[1] = gridcenter[0], res[2] = gridcenter[2];
			return false;
		}
		...
		COMM_FUNC inline T get(TReal x, TReal z)
		{
			...
			gix = min(gix, Nx() - 1);	gix = max(gix, 0);
			giz = min(giz, Ny() - 1);	giz = max(giz, 0);
			int gix_1 = gix + 1; gix_1 = min(gix_1, Nx() - 1);
			int giz_1 = giz + 1; giz_1 = min(giz_1, Ny() - 1);
			...
			return val00 * ((1.0 - fracx)*(1.0 - fracz)) + val01 * ((1.0 - fracx) * fracz) +
				val10 * (fracx * (1.0 - fracz)) + val11 * (fracx * fracz);
		}

		COMM_FUNC inline T& safeGet(int i, int j)
		{
			i = max(i, 0);	i = min(i, Nx() - 1);
			j = max(j, 0);	j = min(j, Ny() - 1);
			return (*this)(i, j);
		}
