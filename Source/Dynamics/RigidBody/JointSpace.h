...
namespace PhysIKA
{
	...
	{
	public:
		COMM_FUNC virtual unsigned int rows() const { return 6; }
		//virtual unsigned int cols() const { return Dof; }

		
		COMM_FUNC virtual const SpatialVector<T> operator*(const VectorBase<T>& q)const { return SpatialVector<T>(); }
		COMM_FUNC virtual const SpatialVector<T> mul(const T* q)const { return SpatialVector<T>(); }

		COMM_FUNC virtual void transposeMul(const SpatialVector<T>& v, T* res)const{}

		COMM_FUNC virtual int dof()const { return 0; }
	...
	public:
		...
		COMM_FUNC virtual unsigned int cols() const { return Dof; }
		...
		COMM_FUNC virtual int dof()const { return Dof; }
		COMM_FUNC virtual const SpatialVector<T>* getBases()const { return m_data; }
		COMM_FUNC virtual SpatialVector<T>* getBases() { return m_data; }
