    ...
    */
    ...
    public:
        ...
        COMM_FUNC PBDJoint() :
            body0(0), body1(0),
            compliance(0),
            rotationXLimited(true), minAngleX(0), maxAngleX(0),
            rotationYLimited(true), minAngleY(0), maxAngleY(0),
            rotationZLimited(true), minAngleZ(0), maxAngleZ(0),
            beContact(false), maxDistance(0)
        {}
        ...
        */
        COMM_FUNC void limitAngle(const Vector<TReal, 3>& n, const Vector<TReal, 3>& a, const Vector<TReal, 3>& b,
            TReal minAngle, TReal maxAngle, TReal dt, TReal maxCorr = CUDART_PI)
        {
    ...
    private:
        ...
        */
        COMM_FUNC TReal applyBodyPairCorrection(const Vector<TReal, 3>& corr, TReal dt, 
            bool beposition = true, TReal maxlambda=-1.0, bool velocityLevel=false, TReal lastLambda=0.0, bool magnitudePositive = true)
        {
            TReal C = corr.norm();
            if (C <= 0)
                return 0.0;
