...
namespace PhysIKA
{
	...
	__global__ void calculateGridForce(DeviceGrid2Df grid,
		DeviceArray<Vector3f> gridforce, DeviceArray<Vector3f> gridtorque,
	...
	bool HeightFieldTerrainRigidInteractionNode::_calcualteSingleContactForce(std::shared_ptr<RigidBody2<DataType3f>> prigid, Vector3f & force, Vector3f & torque)
	{
		...
		calculateGridForce << <gdim, bdim >> > (m_heightField, m_interactForce, m_interactTorque,
			*triangles, vertices,
			rigidinfo, terrainInfo
			);
		//cudaDeviceSynchronize();				
		cudaThreadSynchronize();
	...
	bool HeightFieldTerrainRigidInteractionNode::_test(Real dt)
	{
		...
		for (int i = 0; i < allRigids.size(); ++i)
		{
			...
			if (detectionMethod == HFDETECTION::FACEVISE)
			{
				...
				calculateDepth << <gdim, bdim >> > (m_heightField, m_depthScratch,
					*triangles, vertices,
					rigidinfo
					);
				cudaThreadSynchronize();
				...
				maxdep = thrust::reduce(thrust::device, m_depthScratch.begin(), m_depthScratch.begin() + m_nx * m_ny,
					ContactPointX(),
					thrust::maximum<ContactPointX>());
			}
			else
			{
				...
				calculateDepthPerRigidPoint << <gdim, bdim >> > (m_heightField, m_depthScratch,
					vertices,
					rigidinfo
					);
				cudaThreadSynchronize();
				...
				maxdep = thrust::reduce(thrust::device, m_depthScratch.begin(), m_depthScratch.begin() + npoint,
					ContactPointX(),
					thrust::maximum<ContactPointX>());
			}
			...
			if (maxdep.depth > 0)
			{
				...
				{
					int fnid = contactN - 1;
					ContactPointX contactt1 = maxdep; contactt1.beFriction = fnid;
					ContactPointX contactt2 = maxdep; contactt2.beFriction = fnid;

					Vector3f curnormal(maxdep.normal.x, maxdep.normal.y, maxdep.normal.z);
					Vector3f dirt1(0, 0, -1); dirt1 = rigidinfo.rotation.rotate(dirt1);
		...
		// Solve LCP.
		bool succ = btSolveDantzigLCP(contactN, m_dantzigInput.A.begin(),
			m_dantzigInput.x.begin(),
			m_dantzigInput.b.begin(),
			m_dantzigInput.w.begin(),
			0,
			m_dantzigInput.lo.begin(), m_dantzigInput.hi.begin(),
			m_dantzigInput.findex.begin(),
			m_dantzigscratch);
