...
namespace PhysIKA
{
    bool PBDCar::build()
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            steeringJoint.localPose1.rotation = (Quaterniond(tmpq[0], tmpq[1], tmpq[2], tmpq[3])*
                steeringJoint.localPose0.rotation).normalize();
            steeringJoint.compliance = 0.000000;

            steeringJoint.rotationYLimited = true;
            steeringJoint.minAngleY = 0;    steeringJoint.maxAngleY = 0;
            steeringJoint.rotationXLimited = true;
            if (i < 2)
            {
                steeringJoint.minAngleX = steeringLowerBound;    steeringJoint.maxAngleX = steeringUpperBound;
            }
            else
            {
                steeringJoint.minAngleX = 0; steeringJoint.maxAngleX = 0;
            }
            ...
            wheelJoint.minAngleY = 0;    wheelJoint.maxAngleY = 0;
