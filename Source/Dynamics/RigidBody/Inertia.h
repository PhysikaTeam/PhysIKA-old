...
namespace PhysIKA
{
	...
	public:
		...
		Inertia() :m_inertiaDiagonal(0, 0, 0), m_mass(0) {}
		Inertia(const T& mass, const Vector<T, 3>& inertaiDiagonal) :m_inertiaDiagonal(inertaiDiagonal), m_mass(mass) {}

		virtual unsigned int rows()const { return 6; }
		virtual unsigned int cols()const { return 6; }

		//virtual T& operator()(unsigned int i, unsigned int j);
		virtual const T operator()(unsigned int i, unsigned int j)const;

		const Vector<T, 3>& getInertiaDiagonal()const { return m_inertiaDiagonal; }
		Vector<T, 3>& getInertiaDiagonal() { return m_inertiaDiagonal; }

		const T getMass()const { return m_mass; }

		void setInertiaDiagonal(const Vector<T, 3>& inertiaDiagonal) { m_inertiaDiagonal = inertiaDiagonal; }
		void setMass(const T& mass) { m_mass = mass; }
