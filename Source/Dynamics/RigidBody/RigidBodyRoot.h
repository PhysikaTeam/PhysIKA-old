...
namespace PhysIKA
{
	...
	*/
	...
	public:
		...
		virtual const std::vector<std::shared_ptr<RigidBody2<TDataType>>>& getAllNode() const { return m_all_childs_nodes; }

		// get all descendant nodes in pair
		// pair: first: parent id in the vector; second: shared_ptr of node
		virtual const std::vector< std::pair<int, std::shared_ptr<RigidBody2<TDataType>>>>& getAllParentidNodePair() const { return m_all_childs_node_pairs; }
		...
		const Vector3f& getGravity() const { return m_state->m_gravity; }
		void setGravity(const Vector3f& g) { m_state->m_gravity = g; }

		int getJointDof() const { return m_joint_dof; }
		const std::vector<int>& getJointIdxMap() const { return m_joint_idx_map; }

		std::shared_ptr<SystemState> getSystemState() { return m_state; }
