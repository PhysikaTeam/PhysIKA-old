...
namespace PhysIKA
{
    ...
    */
    ...
    public:
        ...
        const Vector3f& getRelativeR()const { return m_relativePos; }
        Vector3f& getRelativeR() { return m_relativePos; }
        void setRelativeR(const Vector3f& relpos) { m_relativePos = relpos; }

        const Quaternion<float>& getRelativeQ()const { return m_relativeRot; }
        Quaternion<float>& getRelativeQ() { return m_relativeRot; }
        void setRelativeQ(const Quaternion<float>& qua) { m_relativeRot = qua; }

        void setLinearVelocity(const Vector3f& linv) { m_globalLinVel = linv; }
        const Vector3f& getLinearVelocity()const { return m_globalLinVel; }
        Vector3f& getLinearVelocity(){ return m_globalLinVel; }

        void setAngularVelocity(const Vector3f& angv) { m_globalAngVel = angv; }
        const Vector3f& getAngularVelocity()const { return m_globalAngVel; }
        Vector3f& getAngularVelocity() { return m_globalAngVel; }

        void addExternalForce(const Vector3f& force) { m_externalForce += force; }
        void setExternalForce(const Vector3f& force) { m_externalForce = force; }
        const Vector3f& getExternalForce()const { return m_externalForce; }
        Vector3f& getExternalForce() { return m_externalForce; }

        void addExternalTorque(const Vector3f& torque) { m_externalTorque += torque; }
        void setExternalTorque(const Vector3f& torque) { m_externalTorque = torque; }
        const Vector3f& getExternalTorque()const { return m_externalTorque; }
        Vector3f& getExternalTorque() { return m_externalTorque; }

        inline std::shared_ptr<Frame<TDataType>> getTransformationFrame() { return m_frame; }

        float getRadius()const { return m_boundingRadius; }
        void setRadius(float radius) { m_boundingRadius = radius; }
        ...
        void setLinearDamping(float damp) { m_linearDamping = damp; }
        float getLinearDamping() { return m_linearDamping; }
        void setAngularDamping(float damp) { m_angularDamping = damp; }
        float getAngularDamping() { return m_angularDamping; }

        //void integrateForce(Real dt);

        int getCollisionFilterGroup() { return m_collisionGroup; }
        void setCollisionFilterGroup(int group) { m_collisionGroup = group; }

        int getCollisionFilterMask() { return m_collisionMask; }
        void setCollisionFilterMask(int mask) { m_collisionMask = mask; }
