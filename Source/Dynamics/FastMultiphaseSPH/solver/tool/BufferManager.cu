...
void BufferManager::setupDeviceSortHandlers() {
	...
	cudaMemcpy(d_sort_handlers, h_sort_handlers.data(),
		sizeof(SortHandler) * h_sort_handlers.size(),
...
__global__ 
void reorder_by_handlers(SortHandler* handlers,
	uint* dataIdx,
	int num_handler, 
	int num_p) {
	...
	for (int i = 0; i < num_handler; i++) { //for each handler
...
__global__
void reorderDataForward(SortHandler* handlers,
