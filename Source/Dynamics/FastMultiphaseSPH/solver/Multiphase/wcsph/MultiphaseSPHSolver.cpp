...
namespace msph {
	...
	void MultiphaseSPHSolver::dumpRenderData() {
		...
		for (int i = 0; i < num_particles; i++) {
			...
			fprintf(fp, "%f %f %f %d", vol_frac[i * h_param.numTypes], vol_frac[i * h_param.numTypes + 1], vol_frac[i * h_param.numTypes + 2],
				type[i]);
			fprintf(fp, "\n");
		}
		fclose(fp);
	...
	inline bool outside(cfloat3& x, cfloat3& lowbound, cfloat3& upbound) {
		return x.x < lowbound.x || x.y < lowbound.y || x.z < lowbound.z ||
			x.x > upbound.x || x.y > upbound.y || x.z > upbound.z;
	}
