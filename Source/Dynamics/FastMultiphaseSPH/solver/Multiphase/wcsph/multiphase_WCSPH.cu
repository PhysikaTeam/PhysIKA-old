...
namespace msph {
		...
		for (int z = -1; z <= 1; z++)
			for (int y = -1; y <= 1; y++)
				for (int x = -1; x <= 1; x++) {
					...
					if (gridHash == GRID_UNDEF) continue;
	...
	__global__ void UpdateParticleState(int num_particles, MultiphaseParam* param) 
	{
		...
		sigma = shearStress;
		if (pressure < 0) pressure = 0; // tensile crack
		sigma[0][0] -= pressure;
		sigma[1][1] -= pressure;
		sigma[2][2] -= pressure;
		...
		forEachNeighbor(pos, param, [&](int j)
		{
			...
			if (d_data.type[j] == TYPE_RIGID) {
				...
				if (xv < 0) {
					fVisc += nablawij * (
						xv / (d * d + 0.0025f * sr2) *
						param->boundary_visc * sr / d_data.density[index] * massj
						);
				}
			}
		});
	...
	__global__ void computeDriftVel(int num_particles, MultiphaseParam* param) {
		...
		if (d_data.type[index] != TYPE_FLUID) return;
		...
		for (int k = 0; k < param->numTypes; k++) { //for each phase
	...
	__global__ void compupteInterPhaseTensor(int num_particles, MultiphaseParam* param) {
		...
		for (int k = 0; k < param->numTypes; k++) { //for each phase
			if (vol_frac.data[k] < EPSILON) continue;
	...
	__global__ void computePhaseDiffusion(int num_particles, MultiphaseParam* param) {
		...
		forEachNeighbor(xi, param, [&](int j) {
			if (d_data.type[j] != TYPE_FLUID || j == index) return;
			...
			for (int k = 0; k < num_type; k++) {
				...
				if (d_data.vFrac[index].data[k] + tmpk < 0 ||
					d_data.vFrac[j].data[k] - tmpk < 0) {
	...
	void computePhaseDiffusion_host(SimDataMultiphase& data, int num_particles, int num_types) {
		...
		cudaMemcpy(dbg_pt, data.vFrac, num_particles * num_types * sizeof(float),
	...
	__global__ void RigidParticleVolume(int num_particles, MultiphaseParam* param) {
		...
		forEachNeighbor(xi, param, [&](int j) {
			j = d_data.particleIndex[j];
			if (d_data.type[j] != TYPE_RIGID) return;
