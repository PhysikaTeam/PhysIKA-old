...
struct UnboundedGrid2 {
    ...
    UnboundedGrid2(){}
    UnboundedGrid2(Vec<real, 2> spacing) : spacingInv(1/spacing.x, 1/spacing.y) {}
...
public:
    ...
    void sort(size_t ptsNum, const Vec<real, 2> * pos) {
        using ParticlePack = struct { HashId hashId; size_t particleId; };
        ...
        std::sort(sortBuffer.begin(), sortBuffer.end(),
                  [](const ParticlePack& a, const ParticlePack& b) {
    ...
    size_t * getPermutation() { return ids.data(); }
    template <typename Op>
    void forEachIndexInCell(
        Cell cell, Op op) const {
