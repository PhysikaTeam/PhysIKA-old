...
template <typename T>
class DenseField : public Field, public std::vector<T> {
public:
  using BaseType = std::vector<T>;
private:
  size_t size() { return BaseType::size(); }
  void resize(size_t s) { BaseType::resize(s); }
...
public:
  ...
  void resize(size_t s) {
    ...
    while (_size > id.size()) id.push_back(++idCount);
...
private:
  ...
  typename T::BaseType &getField(const std::string &name) {
    if (find(name) == end())
      throw std::runtime_error("DataFrame: field \"" + name +
                               "\" does not exist!");
