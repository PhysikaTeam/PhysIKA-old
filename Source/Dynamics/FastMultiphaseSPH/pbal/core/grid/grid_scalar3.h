    ...
    class GridScalar3 {
        ...
        public:
            ...
            void resize(Size3 sz, Vec3<T> sp, Vec3<T> ori, T initVal = T()) {
                ...
                _spacing = sp;
                _origin = ori + Vec3<T>(
                    static_cast<T>(0.5 * sp.x),
                    static_cast<T>(0.5 * sp.y),
                    static_cast<T>(0.5 * sp.z));
                _data.resize(_size.x * _size.y * _size.z, initVal);
            }

            Vec3<T> dataPosition(int i, int j, int k) const {
                return _origin + _spacing * Vec3<T>(
                        static_cast<T>(i),
                        static_cast<T>(j),
                        static_cast<T>(k));
            }
            ...
            T sample(const Vec3<T>& pt) const {
                auto val = ConstArrayPtr3<T>(_data.data(), _size).linearSample(
                    pt,
                    _spacing,
                    _origin
                );
