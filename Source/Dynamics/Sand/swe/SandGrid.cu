...
namespace PhysIKA
{
	...
	__global__ void g_initGrid(float4 *grid, int pitch, float *wavegrid, int wavepitch, float* landgrid, int landpitch,
	...
	void SandGrid::initialize(float *land_height, float *sand_height)
	{
		...
		cudaMemcpy2D(initSurface.GetDataPtr(), initSurface.Pitch() * sizeof(float), sand_height, Nx * sizeof(float),
			Nx * sizeof(float), Ny, cudaMemcpyHostToDevice);

		// copy land data to GPU
		DeviceArrayPitch2D1f landHeight;
		landHeight.Resize(Nx, Ny);
		cudaMemcpy2D(landHeight.GetDataPtr(), landHeight.Pitch() * sizeof(float), land_height, Nx * sizeof(float),
			Nx * sizeof(float), Ny, cudaMemcpyHostToDevice);
		...
		g_initGrid << < gridDims, blockDims >> > (m_sandGrid.GetDataPtr(), m_sandGrid.Pitch(),
			initSurface.GetDataPtr(), initSurface.Pitch(),
			landHeight.GetDataPtr(), landHeight.Pitch(),
			Nx, Ny);
		cuSynchronize();
	...
	)
	{
		...
		if (tidx >= grid.Nx() || tidy >= grid.Ny())return;
	...
	{
		if (m_sandHeight.Nx() != m_sandGrid.Nx() ||
			m_sandHeight.Ny() != m_sandGrid.Ny())
		{
			m_sandHeight.resize(m_sandGrid.Nx(), m_sandGrid.Ny());
		}

		uint3 gsize = { m_sandGrid.Nx(), m_sandGrid.Ny(), 1 };
		cuExecute2D(gsize, SandGrid_updateSandGridHeight,
			m_sandHeight, m_sandGrid
		);
	}
	...
	{
		if (m_landHeight.Nx() != m_sandGrid.Nx() ||
			m_landHeight.Ny() != m_sandGrid.Ny())
		{
			m_landHeight.resize(m_sandGrid.Nx(), m_sandGrid.Ny());
		}

		uint3 gsize = { m_sandGrid.Nx(), m_sandGrid.Ny(), 1 };
		cuExecute2D(gsize, SandGrid_updateLandGridHeight,
			m_landHeight, m_sandGrid
		);
	}
	...
	void SandGrid::setSandGridHeight(DeviceHeightField1d & sandheight)
	{
		...
		uint3 gsize = { m_sandGrid.Nx(), m_sandGrid.Ny(), 1 };
		cuExecute2D(gsize, SandGrid_setSandHeightToFloat4,
			m_sandGrid, m_sandHeight
		);
	}
	...
	void SandGrid::setSandGridHeight(double * sandheight)
	{
		...
		cuSafeCall(
			cudaMemcpy2D(m_sandHeight.GetDataPtr(), m_sandHeight.Pitch() * sizeof(double),
				sandheight, width,
				width, m_sandHeight.Ny(), cudaMemcpyHostToDevice
			)
		);
	...
	void SandGrid::getSandGridHeight(double * sandheight)
	{
		int width = m_sandHeight.Nx() * sizeof(double);
		cuSafeCall(
			cudaMemcpy2D(sandheight, width,
				m_sandHeight.GetDataPtr(), m_sandHeight.Pitch() * sizeof(double),
				width, m_sandHeight.Ny(), cudaMemcpyDeviceToHost
			)
		);
	}
