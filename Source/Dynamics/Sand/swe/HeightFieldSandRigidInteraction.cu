...
namespace PhysIKA
{
	...
	__global__ void HFSRI_updateGridParticleInfo(
		...
		int minGx, int minGz, int sizeGx, int sizeGz
	)
	{
		int tid = threadIdx.x + blockDim.x * blockIdx.x;
		if (tid >= (sizeGx * sizeGz))return;
		...
		parMass[tid] = sandinfo.sandRho * sandinfo.griddl * sandinfo.griddl *
			sandheight(gi, gj);
	}
	...
	)
	{
		...
		if (tidx >= sandinfo.nx || tidy >= sandinfo.ny)return;
	...
	void HeightFieldSandRigidInteraction::_updateGridParticleInfo(int i)
	{
		...
		cuExecute(gsize.x * gsize.y, HFSRI_updateGridParticleInfo,
			m_gridParticle, m_gridVel, m_gridMass,
			*m_psandInfo,
			*m_sandHeight, *m_landHeight,
			m_minGi, m_minGj, m_sizeGi, m_sizeGj
		);
	}
	...
	void HeightFieldSandRigidInteraction::_computeBoundingGrid(int & minGx, int & minGz, int & maxGx, int & maxGz, float radius, const Vector3f & center)
	{
		...
		minGx = minG.x - 3;	minGx = max(minGx, 0);
		minGz = minG.y - 3; minGz = max(minGz, 0);

		Vector3d maxp(center[0] + radius, center[1], center[2] + radius);
		int2 maxG = m_sandHeight->gridIndex(maxp);
		maxGx = maxG.x + 3;	maxGx = min(maxGx, m_sandHeight->Nx());
		maxGz = maxG.y + 3; maxGz = min(maxGz, m_sandHeight->Ny());
	}
}