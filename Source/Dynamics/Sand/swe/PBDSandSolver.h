...
namespace PhysIKA
{
	...
	public:
		...
		virtual float getMaxTimeStep()override { return 0.02; }
		...
		void setParticles(Vector3d* pos, Vector3d* vel, double* mass,
		...
		DeviceDArray<Vector3d>& getParticleVelocity() { return m_particleVel; }
		const DeviceDArray<Vector3d>& getParticleVelocity()const { return m_particleVel; }

		DeviceDArray<Vector3d>& getParticlePosition() { return m_particlePos; }
		const DeviceDArray<Vector3d>& getParticlePosition()const { return m_particlePos; }

		DeviceDArray<Vector3d>& getParticlePosition3D() { return m_particlePos3D; }
		const DeviceDArray<Vector3d>& getParticlePosition3D()const { return m_particlePos3D; }

		DeviceDArray<double>& getParticleRho2D(){ return m_particleRho2D; }
		const DeviceDArray<double>& getParticleRho2D() const { return m_particleRho2D; }
		
		DeviceDArray<double>& getParticleMass() { return m_particleMass; }
		const DeviceDArray<double>& getParticleMass() const { return m_particleMass; }

		DeviceDArray<ParticleType>& getParticleTypes() { return m_particleType; }
		const DeviceDArray<ParticleType>& getParticleTypes() const { return m_particleType; }

		NeighborField<int>& getNeighborField() { return m_neighbors; }
		NeighborList<int>& getNeighborList() { return m_neighbors.getValue(); }
		//const NeighborList<int>& getNeighborList() const { return m_neighbors.getValue(); }

		DeviceHeightField1d& getLand() { return m_land; }
		const DeviceHeightField1d& getLand()const { return m_land; }


		double getRho0() { return m_rho0; }
		double getSmoothingLength() { return m_smoothingLength.getValue(); }

		double getM0() { return m_m0; }

		void needPosition3D(bool needpos3d) { m_need3DPos = needpos3d; }


		void setFlowingLayerHeight(double h) { m_flowingLayerHeight = h; }
		double getFlowingLayerHeight()const { return m_flowingLayerHeight; }


		void setPostInitializeFun(PostInitFun fun) { m_postInitFun = fun; }
