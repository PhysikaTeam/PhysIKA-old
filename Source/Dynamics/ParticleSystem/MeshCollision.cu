    ...
    for (int ne = 0; ne < nbrSize; ne++)
    {
        ...
        if (ne < nbrSize - 1 && triangle_neighbors.getElement(pId, ne + 1) < 0)
        {
            ...
            do
            {
                ...
                if ((t3d.normal().cross(t3d_n.normal())).norm() > EPSILON * t3d_n.normal().norm() * t3d.normal().norm())
                    break;
...
void MeshCollision<TDataType>::doCollision()
{
    ...
    cuExecute(total_num, VC_Sort_Neighbors_Collide, m_position.getValue(), m_triangle_index.getValue(), m_triangle_vertex.getValue(), m_neighborhood_tri.getValue());
    cuSynchronize();
