...
PhysIKA::SemiAnalyticalSFINode<TDataType>::SemiAnalyticalSFINode(std::string name)
    : Node(name)
{
...
bool SemiAnalyticalSFINode<TDataType>::resetStatus()
{
    ...
    if (total_num > 0)
    {
        ...
        for (int i = 0; i < m_particleSystems.size(); i++)
        {
            ...
            cudaMemcpy(m_particle_velocity.getValue().getDataPtr() + start, vels.getDataPtr(), num * sizeof(Coord), cudaMemcpyDeviceToDevice);
    ...
    for (int i = 0; i < m_surfaces.size(); i++)
    {
        ...
        cudaMemcpy(m_triangle_vertex.getValue().getDataPtr() + start_vertex, vertex_position.getDataPtr(), num_vertex * sizeof(Coord), cudaMemcpyDeviceToDevice);
        cudaMemcpy(m_triangle_vertex_old.getValue().getDataPtr() + start_vertex, vertex_position.getDataPtr(), num_vertex * sizeof(Coord), cudaMemcpyDeviceToDevice);
        ...
        //printf("%d\n", start_vertex);
        cudaMemcpy(m_triangle_index.getValue().getDataPtr() + start_triangle, host_triangle.getDataPtr(), num_triangle * sizeof(Triangle), cudaMemcpyHostToDevice);
...
void SemiAnalyticalSFINode<TDataType>::advance(Real dt)
{
    ...
    if (m_particle_position.getElementCount() > 0)
    {
        ...
        for (int i = 0; i < m_particleSystems.size(); i++)
        {
            ...
            DeviceArray<Coord>& vels   = m_particleSystems[i]->currentVelocity()->getValue();
            int                 num    = points.size();
            cudaMemcpy(points.getDataPtr(), allposs.getDataPtr() + start, num * sizeof(Coord), cudaMemcpyDeviceToDevice);
            cudaMemcpy(vels.getDataPtr(), allvels.getDataPtr() + start, num * sizeof(Coord), cudaMemcpyDeviceToDevice);
