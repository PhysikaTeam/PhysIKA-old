    ...
    Real                horizon)
{
    ...
    for (int ne = 0; ne < size_i; ne++)
    {
        ...
        if (r > EPSILON)
        {
            ...
            curM(0, 0) += p[0] * p[0] * weight;
            curM(0, 1) += p[0] * p[1] * weight;
            curM(0, 2) += p[0] * p[2] * weight;
            curM(1, 0) += p[1] * p[0] * weight;
            curM(1, 1) += p[1] * p[1] * weight;
            curM(1, 2) += p[1] * p[2] * weight;
            curM(2, 0) += p[2] * p[0] * weight;
            curM(2, 1) += p[2] * p[1] * weight;
            curM(2, 2) += p[2] * p[2] * weight;

            refM(0, 0) += q[0] * p[0] * weight;
            refM(0, 1) += q[0] * p[1] * weight;
            refM(0, 2) += q[0] * p[2] * weight;
            refM(1, 0) += q[1] * p[0] * weight;
            refM(1, 1) += q[1] * p[1] * weight;
            refM(1, 2) += q[1] * p[2] * weight;
            refM(2, 0) += q[2] * p[0] * weight;
            refM(2, 1) += q[2] * p[1] * weight;
            refM(2, 2) += q[2] * p[2] * weight;
    ...
    Real                smoothingLength)
{
    ...
    for (int ne = 0; ne < size_i; ne++)
    {
        ...
        if (r > EPSILON)
        {
            ...
            mat_i(0, 0) += p[0] * q[0] * weight;
            mat_i(0, 1) += p[0] * q[1] * weight;
            mat_i(0, 2) += p[0] * q[2] * weight;
            mat_i(1, 0) += p[1] * q[0] * weight;
            mat_i(1, 1) += p[1] * q[1] * weight;
            mat_i(1, 2) += p[1] * q[2] * weight;
            mat_i(2, 0) += p[2] * q[0] * weight;
            mat_i(2, 1) += p[2] * q[1] * weight;
            mat_i(2, 2) += p[2] * q[2] * weight;

            invK_i(0, 0) += q[0] * q[0] * weight;
            invK_i(0, 1) += q[0] * q[1] * weight;
            invK_i(0, 2) += q[0] * q[2] * weight;
            invK_i(1, 0) += q[1] * q[0] * weight;
            invK_i(1, 1) += q[1] * q[1] * weight;
            invK_i(1, 2) += q[1] * q[2] * weight;
            invK_i(2, 0) += q[2] * q[0] * weight;
            invK_i(2, 1) += q[2] * q[1] * weight;
            invK_i(2, 2) += q[2] * q[2] * weight;
