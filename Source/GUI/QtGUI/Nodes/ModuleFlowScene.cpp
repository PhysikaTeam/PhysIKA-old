  ...
  , _registry(std::move(registry))
{
  ...
  connect(this, &ModuleFlowScene::nodeMoved, this, &ModuleFlowScene::moveModulePosition);
}
...
// 	bunny->getSurfaceNode()->addVisualModule(renderer);
	...
	auto addModuleWidget = [&](Module* m) -> void
	{
		...
		auto type = std::make_unique<ModuleWidget>(m);
...
void ModuleFlowScene::showNodeFlow(Node* node)
{
	clearScene();
...
void ModuleFlowScene::moveModulePosition(QtNode& n, const QPointF& newLocation)
{
	ModuleWidget* mw = dynamic_cast<ModuleWidget*>(n.nodeDataModel());
