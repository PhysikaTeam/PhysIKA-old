        ...
        if (const QStyleOptionToolButton* toolbutton = static_cast<const QStyleOptionToolButton*>(opt))
        {
            ...
            else
            {
                ...
                if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly)
                {
                    ...
                    proxy()->drawItemText(p, QStyle::visualRect(opt->direction, rect, tr), alignment, toolbutton->palette, toolbutton->state & State_Enabled, toolbutton->text, QPalette::ButtonText);
                }
                else
                {
                    rect.translate(shiftX, shiftY);
