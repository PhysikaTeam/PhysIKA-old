...
{
	// add by HNU
	void vtk2ObjThread::doWork(QString outputFileUrl)
	{
		if (outputFileUrl == "")
			return;
...
####\n";
		...
		emit resultReady(urlSplit.join("/"));
	}
...
####\n";
	...
	int vtkNums;
	int QTextFieldWidget::multiK2Obj(QString showFileUrl)
	{
		if (showFileUrl == "")
		{
	...
	int QTextFieldWidget::startAnimating(QString showFileUrl)
	{
		...
		if (objList.size() < vtkNums)
			multiK2Obj(showFileUrl);

		//	if (objList.size() < vtkNums)
		//	{
		//		for (int i = 0; i < vtkNums; i++)
		//		{
		//			vtk2ObjThread* worker = new vtk2ObjThread;
		//			QThread* workThread = new QThread;
		//			worker->moveToThread(workThread);
		//			connect(workThread, &QThread::finished, worker, &QObject::deleteLater);
		//			connect(this, &QTextFieldWidget::multiThread, worker, &vtk2ObjThread::doWork);
		//			workThread->start();
		//			emit multiThread(showFileUrl);
		//		}
		//	}
