...
//#include "Core/Algorithm/svd3_cuda.h"
    ...
    Real               smoothingLength)  //radius
{
    ...
    for (int ne = 0; ne < nbSize; ne++)
    {
        ...
        if (r1 > EPSILON)
        {
            Real  weight1 = g_weightKernel.Weight(r1, smoothingLength);
        ...
        if (r2 > EPSILON)
        {
            Real weight2 = g_weightKernel.Weight(r2, smoothingLength);

            Coord p  = (from[j] - to[pId]) / smoothingLength;
            Coord q2 = (initFrom[j] - initTo_i) / smoothingLength * weight2;

            deform_i(0, 0) += p[0] * q2[0];
            deform_i(0, 1) += p[0] * q2[1];
            deform_i(0, 2) += p[0] * q2[2];
            deform_i(1, 0) += p[1] * q2[0];
            deform_i(1, 1) += p[1] * q2[1];
            deform_i(1, 2) += p[1] * q2[2];
            deform_i(2, 0) += p[2] * q2[0];
            deform_i(2, 1) += p[2] * q2[1];
            deform_i(2, 2) += p[2] * q2[2];
    ...
    for (int ne = 0; ne < nbSize; ne++)
    {
        ...
        {
            Real weight = WeightRRR(r, smoothingLength);
...
__device__ Real WeightRRR(Real r, Real h)
{
    SmoothKernel<Real> kernSmooth;
    Real               q = r / h;
    return kernSmooth.Weight(r, h) / (h * h * h * h);
