...
namespace PhysIKA {
    ...
    void Collision::transformMesh(unsigned int numVtx, unsigned int numTri, 
        std::vector<unsigned int> tris, std::vector<float> vtxs,
        std::vector<float> p_vtxs, int m_id, bool able_selfcollision
    ) {
    ...
    void Collision::transformMesh(unsigned int numVtx, unsigned int numTri, 
        std::vector<unsigned int> tris, std::vector<vec3f> vtxs,
        std::vector<vec3f> p_vtxs, int m_id, bool able_selfcollision
    ) {
        CollisionMesh::tri3f* _tris = new CollisionMesh::tri3f[numTri];
        vec3f* _vtxs = new vec3f[numVtx];
        vec3f* pre_vtxs = new vec3f[numVtx];
