...
__device__ void tri_DCD(
	...
	float3* cx, float3* cx0, int2* pairRets, int4* dv, int* VF_EE, float* dist, int* CCDres, int* t, uint* pairIdx, int fid1, int fid2, float* thickness
)
{
	float3 p[3];
	p[0] = cx[t1];
...
__global__ void kernelGetCollisions(
	int2* pairs, int num,
	float3* cx, float3* cx0, tri3f* ctris, int2* pairRets, int4* dv, int* VF_EE, float* dist, int* CCDres, uint* pairIdx, int* t, float* thickness,
	int stride)
{
