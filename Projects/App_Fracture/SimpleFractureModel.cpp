...
namespace Physika
{
	IMPLEMENT_CLASS_1(SimpleFractureModel, TDataType)

	template<typename TDataType>
	SimpleFractureModel<TDataType>::SimpleFractureModel()
		: NumericalModel()
		, m_restRho(Real(1000))
		, m_pNum(0)
	{
	...
	template<typename TDataType>
	bool SimpleFractureModel<TDataType>::initializeImpl()
	{
...
// 		m_mapping->initialize(*(m_position.getReference()), (pSet->getPoints()));
	...
	template<typename TDataType>
	void SimpleFractureModel<TDataType>::step(Real dt)
	{
		Node* parent = getParent();
		if (parent == NULL)
		{
	...
	template<typename TDataType>
	void SimpleFractureModel<TDataType>::setIncompressibilitySolver(std::shared_ptr<ConstraintModule> solver)
	{
	...
	template<typename TDataType>
	void SimpleFractureModel<TDataType>::setViscositySolver(std::shared_ptr<ConstraintModule> solver)
	{
	...
	template<typename TDataType>
	void SimpleFractureModel<TDataType>::setSurfaceTensionSolver(std::shared_ptr<ForceModule> solver)
	{
