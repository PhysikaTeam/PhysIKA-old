...
import ComputeHistogramWorker from 'vtk.js/Sources/Interaction/Widgets/PiecewiseGaussianWidget/ComputeHistogram.worker'
...
function vtkPiecewiseGaussianWidget(publicAPI, model) {
  ...
  publicAPI.setDataArray = (
    ...
    while (arrayIndex < array.length) {
      ...
      //
      webworkerPromiseArray.push(workerPromise);
    ...
    Promise.all(workers).then((workerResults) => {
      ...
      //
      webworkerPromiseArray.forEach( webworkerPromise=>{
        webworkerPromise.terminate();
      });
    });
