/*
* @file cubic_mesh_render_task_test.cpp
...
#include "Physika_Render/Volumetric_Mesh_Render/Cubic_Mesh_Render/cubic_mesh_render_util.h"
#include "Physika_Render/Volumetric_Mesh_Render/Cubic_Mesh_Render/cubic_mesh_point_render_task.h"
#include "Physika_Render/Volumetric_Mesh_Render/Cubic_Mesh_Render/cubic_mesh_point_vector_render_task.h"
#include "Physika_Render/Volumetric_Mesh_Render/Cubic_Mesh_Render/cubic_mesh_wireframe_render_task.h"
#include "Physika_Render/Volumetric_Mesh_Render/Cubic_Mesh_Render/cubic_mesh_solid_render_task.h"
...
void initFunction()
{
    ...
    auto render_util = make_shared<CubicMeshRenderUtil<double>>(mesh);
    //---------------------------------------------------------------------------------------------------------------------

    //---------------------------------------------------------------------------------------------------------------------
    auto wireframe_render_task = make_shared<CubicMeshWireframeRenderTask<double>>(render_util);
    //render_scene_config.pushBackRenderTask(wireframe_render_task);

    //---------------------------------------------------------------------------------------------------------------------
    auto solid_render_task = make_shared<CubicMeshSolidRenderTask<double>>(render_util);
    render_scene_config.pushBackRenderTask(solid_render_task);

    //---------------------------------------------------------------------------------------------------------------------
    auto point_render_task = make_shared<CubicMeshPointRenderTask<double>>(render_util);
    point_render_task->setPointScaleForPointSprite(30);
    //render_scene_config.pushBackRenderTask(point_render_task);

    //---------------------------------------------------------------------------------------------------------------------
    auto point_vector_render_task = make_shared<CubicMeshPointVectorRenderTask<double>>(render_util);
    point_vector_render_task->setScaleFactor(0.05);
    point_vector_render_task->enableUsePointVectorColor();
    //point_vector_render_task->setPointVectorAsBoundaryNormalVector();
