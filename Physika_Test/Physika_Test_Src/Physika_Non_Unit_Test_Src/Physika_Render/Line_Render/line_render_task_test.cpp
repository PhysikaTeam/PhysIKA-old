/*
* @file line_render_task_test.cpp
* @brief Test line render task of Physika.
* @author WeiChen
*
...
std::vector<Vector3d> getSurfaceMeshLines(SurfaceMesh<double> & mesh)
{
...
void initFunction()
{
    ...
    vector<Color4f> col_vec;
    for (unsigned int i = 0; i < line_render_util->lineNum(); ++i)
        col_vec.push_back(color_map.colorViaRatio(float(i) / line_render_util->lineNum()));
    line_render_task->setLineColors(col_vec);
    //line_render_task->setUniformColor(Color4f::Cyan());

    render_scene_config.pushBackRenderTask(line_render_task);
